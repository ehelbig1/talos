...
use talos_ops_alert_store::{truncate_chars, MAX_KEY_CHARS, MAX_SOURCE_CHARS};
pub use talos_ops_alert_store::{
    validate_severity, IngestOutcome, NewOpsAlert, OpsAlertIngestError, ASSIGNABLE_SEVERITIES,
};
...
impl OpsAlertRepository {
    ...
    /// Normalize + bound a [`NewOpsAlert`] for insertion (delegates to
    /// `talos_ops_alert_store::sanitize`, the one home since package CD).
    #[cfg(test)]
    pub(crate) fn sanitize(a: NewOpsAlert) -> Result<NewOpsAlert, OpsAlertIngestError> {
        talos_ops_alert_store::sanitize(a)
    }

    /// Upsert an alert for `user_id` — see `talos_ops_alert_store::ingest`,
    /// which holds the one `ops_alerts` INSERT (moved there in package CD so
    /// crates below this repository can raise an alert without a dependency
    /// cycle).
    pub async fn ingest(
        &self,
        user_id: Uuid,
        ...
        talos_ops_alert_store::ingest(&self.db_pool, user_id, org_id, alert).await
    }

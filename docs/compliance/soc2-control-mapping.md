...
| CC7.1-06 | Admin event log | Privileged operations recorded in `admin_event_log`, including MCP agent registration, actor LLM-tier / write-ceiling / egress-scope changes, capability-ceiling grants and revocations, API-key creation / rotation / revocation / deletion, module permission changes, workflow and module deletion, and execution pause / resume. MCP agent revocation (`revokeMcpAgent`) writes a log line only, no `admin_event_log` row | `migrations/20260407000001_admin_event_log.sql`, `talos-actor-repository/src/lib.rs` (`spawn_log_admin_event`, `insert_admin_event_log`), `talos-api/src/schema/actors/mutations.rs` |
...
| MCP agent revocation not in admin event log | CC7.1 | `revokeMcpAgent` writes no `admin_event_log` row (registration does) | Write an admin event on revocation | Unassigned |

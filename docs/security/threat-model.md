...
| Audit log tampering | Repudiation | Immutability triggers on all 4 audit tables (`audit_events`, `auth_audit_log`, `secret_audit_log`, `admin_event_log`); BEFORE UPDATE OR DELETE raises SQLSTATE 42501; append-only design | Superuser can DROP trigger (mitigate with pgaudit + separate audit DB user) |
...
| Secret interception on wire | Info Disclosure | Secrets encrypted with AES-256-GCM before placement in JobRequest; separate encryption key per job; worker decrypts only in memory | Encryption key transmitted alongside ciphertext in job (defense-in-depth, not standalone protection) |

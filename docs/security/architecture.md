...
| `admin_event_log` | Admin action events | Triggers: `trg_admin_event_log_immutable` (UPDATE/DELETE), `trg_admin_event_log_immutable_truncate` (TRUNCATE) | Yes: every writer goes through `talos-admin-event-log`, which truncates the summary to 1000 bytes, bounds `details` at 1 MiB and DLP-redacts both (structural lint check 94 keeps the INSERT there) |

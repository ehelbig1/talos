...
- [ ] Metrics: `record_rpc_metric` with split `queue` / `exec` `Duration`s and a distinct outcome for every failure mode (`unauthorized`, `replay`, `invalid`, `timeout`, `storage_full`, `not_found`, `internal`). **Both arguments are typed** — add the new subject to `talos_metrics::rpc::RpcSubject` and list exactly the outcomes YOUR subscriber can pass, because that per-subject list IS the pre-seed loop: an outcome you forget is an ABSENT series (`increase(...) > 0` matches nothing) and one you add speculatively is a seeded pair nothing can increment (check 58). `talos_rpc_subscribers::rpc_instrument_table_pins::the_declared_table_matches_the_source` will fail in whichever direction you got wrong. The outcome also carries its `RpcOutcomeClass`, which decides the LOG LEVEL and is what `TalosRPCSubjectFailing` selects on — a DESIGNED state (a configured cap, a lifecycle position the operator has not misconfigured) is `Declined`/`info!`, not an alarm.
- [ ] **If the op MUTATES on the ACTOR's behalf, gate it on the write ceiling** (`talos_rpc_subscribers::write_ceiling::gate`, lint check 84). A verified signature proves the sender holds `WORKER_SHARED_KEY`, which is FLEET-SHARED — it does NOT prove the sender ran the worker's own gate. Before #754 the memory / integration-state / database RPCs all trusted it to, and a worker booted without `TALOS_WRITE_CEILING_ENFORCED` could write memory for a `readonly` actor. Use the WORKER's own op token (`write_ceiling::CONTROLLER_SERVED_WRITE_OPS`) so `get_module_info.mutation_profile` still correlates one-to-one, resolve the ceiling from the SIGNED `actor_id` only, and fail CLOSED on both "no such actor" and "could not read" — with the two reasons distinct for the operator and COLLAPSED for the caller (a reason-split reply is an actor-existence oracle). Ask the same question of every op you add: is this the ACTOR's mutation, the ENGINE's (`talos.state.write` — deliberately ungated, matching the worker), or the PLATFORM's (a sweeper)?

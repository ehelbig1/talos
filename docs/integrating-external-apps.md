...
| `vault://...` literal arriving inside guest code via `data["config"]["X"]` | You expected node-config vault-substitution. It happens for HTTP header values and for string VALUES of a JSON request body — not for arbitrary config keys. | Either (a) use the value as a header, or put it in the JSON body of a request sent with `Content-Type: application/json` (the worker resolves both on `http::fetch`, `http::fetch_all`, `webhook::send` and `graphql::execute`, and the guest keeps the placeholder either way; body substitution deliberately does not happen where the host composes the body from your fields — `email::send`, object storage, `llm::*`), or (b) use `secrets::get-secret(path)` from inside the module. |

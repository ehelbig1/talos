...
| `vault://...` literal arriving inside guest code via `data["config"]["X"]` | You expected node-config vault-substitution. It happens for HTTP header values and for string VALUES of a JSON request body — not for arbitrary config keys. | Either (a) use the value as a header, or put it in the JSON body of a request sent with `Content-Type: application/json` (the worker resolves both at fetch time and the guest keeps the placeholder either way), or (b) use `secrets::get-secret(path)` from inside the module. |

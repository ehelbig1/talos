...
| [0010](0010-asymmetric-worker-trust-boundary.md) | Asymmetric worker-trust boundary | Draft | The genuine fix for the single fleet-wide `WORKER_SHARED_KEY` (review finding #1, where per-worker HMAC proved to be theater): Ed25519 controller→worker dispatch, per-worker keypairs for worker→controller, envelope sealing (X25519/ECIES or per-execution), staged `crypto_scheme` rollout |

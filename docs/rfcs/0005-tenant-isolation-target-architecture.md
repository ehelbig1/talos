...
| **S2** | **Permissive backstop on the user-facing IDOR surfaces** of hot/cross-cutting tables (the actual attack vector). App-layer stays primary for internal readers. | `workflows` GraphQL ✓✓; **secrets next**; then `workflow_executions`, `actors` |

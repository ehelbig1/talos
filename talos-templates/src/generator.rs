...
impl TemplateGenerator {
    ...
    fn validate_generated_code(&self, code: &str) -> Result<()> {
        ...
        // If it uses the talos_module macro, we assume it's valid as the macro
        // generates the required boilerplate (Guest impl, export!, etc.)
        if code.contains("talos_module!") || code.contains("#[talos_module") {
            // Still check for unresolved placeholders
...
"#;
        ...
        let code = result.expect("Code generation should succeed");

...
/// Helper to create test auth service using testcontainers pool
async fn setup_auth_service() -> AuthService {
    let db_pool = test_helpers::get_test_db_pool().await;
    let jwt_secret = "test-secret-key-for-testing-only-min-32-chars".to_string();
    let bcrypt_cost = 12;
    AuthService::new(db_pool, jwt_secret, bcrypt_cost, None).expect("Failed to create AuthService")
}
...
async fn test_refresh_token_flow() {
    ...
    let (new_access_token, _new_refresh_token, refreshed_user, _is_2fa_verified) = result.unwrap();
...
async fn test_account_lockout_after_failed_attempts() {
    ...
    // r269 intentionally generalised the locked-account error to match
    // the invalid-password message — distinguishable text would let an
    // attacker confirm an email is registered after burning the lockout
    // threshold. Caller can no longer rely on a "locked" string; the
    // server-side log retains the real lockout state for forensics.

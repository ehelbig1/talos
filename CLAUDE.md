...
- **`make lint` enforces structural rules** via `scripts/lint-structural.sh`. 58 checks today (the authoritative, inline-documented list lives in the script; `bash scripts/lint-structural.sh --count` prints the live number, and check 54 fails the lint if this sentence's count goes stale), each tied to a specific past regression so it catches at PR-time the class of bug that survives `cargo check` cleanly but breaks at CI or request time:
  ...
  57. sub-engine built without ceiling narrowing — a sub-engine from `adapter_set().into_engine_with_graph(…)` inherits the PARENT's `max_llm_tier`/`max_write_ceiling` verbatim; a sub-workflow bound to a stricter actor then silently runs at the looser parent ceiling (the H2 escalation PR #504 closed at three sites via `narrow_subengine_ceilings`). Any non-test file calling `into_engine_with_graph` must reference the narrowing chokepoint (`narrow_subengine_ceilings` / `resolve_subworkflow_ceilings`); opt-out `// allow-unnarrowed-subengine: <reason>` for deliberate parent-context clones. Sibling of checks 29/56

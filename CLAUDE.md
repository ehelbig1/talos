...
- **`scripts/publish-images.sh`** is the canonical build path. It mirrors `main-publish.yml`'s contract: builds the three images via `docker compose build controller worker frontend`, pushes `:main-<sha>` (+ `:main-latest`) to `ghcr.io/<owner>/talos-*`, captures digests via `docker inspect`, signs each digest with `cosign sign --yes` (keyless, OAuth identity). Flags: `--no-push`, `--no-sign`, `--service NAME`, `--update-env PATH`. Emits a copy-pasteable `TALOS_*_DIGEST=…` block at the tail for `/etc/talos/install.env`.
...
- **Dirty-tree builds** are allowed but tag-suffixed `-dirty` so they can never be confused with a clean-main image. Don't deploy `-dirty` builds to production.
- **CI gates** (lint, test, structural lint) run locally via `make lint` and `cargo test --workspace`. The user must run these before `bash scripts/publish-images.sh`; nothing automates the gate.

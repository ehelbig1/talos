...
- **`make lint` enforces structural rules** via `scripts/lint-structural.sh`. 40 checks today (the authoritative, inline-documented list lives in the script), each tied to a specific past regression so it catches at PR-time the class of bug that survives `cargo check` cleanly but breaks at CI or request time:

...
- **`make lint` enforces structural rules** via `scripts/lint-structural.sh`. 55 checks today (the authoritative, inline-documented list lives in the script; `bash scripts/lint-structural.sh --count` prints the live number, and check 54 fails the lint if this sentence's count goes stale), each tied to a specific past regression so it catches at PR-time the class of bug that survives `cargo check` cleanly but breaks at CI or request time:
  ...
  55. bare `row.get(`/`r.get(` sqlx reads in DB-layer crates — RATCHET (introduced 2026-07-08 at 473 after #427: a bare `.get` PANICS on NULL/type-drift, killing the tokio task mid-request → caller sees a connection reset; the first workflow-bound webhook's NULL `module_id` took down every `list_webhooks` call this way). The panic-side sibling of check 52 — correct idiom is `try_get(...)?` (same fail-loud, clean error). Scope: repository crates + the check-52 widened DB-layer family (talos-memory, talos-secrets-manager, talos-registry, talos-module-executions, talos-integration-state, talos-auth). Count must never exceed `BARE_ROW_GET_BASELINE` in the script; lower the baseline in the same PR when burning sites down (webhook-repository already at 0)

...
**Closed to new bullets since 2026-09-25**: a package now writes its record to its own file under `docs/engineering-log/packages/` (see "Rules for adding to this file" at the top of this section); the bullets below stay as the record up to that date, and EP's per-bullet byte budget no longer applies.
...
* **ES, 2026-09-25 — an API key could mint a verified session; bcrypt ran on the runtime thread; the capability bootstrap re-armed.** (1) Keys carry `IsTwoFactorVerified(true)`, so a `workflows:read` key could `enableTwoFactor` its own secret and `verifyTwoFactor` into a `SecondFactorVerified` session. Now setup/enable/verify refuse keys (`SecondFactorRefusal::ApiKey`), verify requires a PENDING session (`NO_PENDING_SECOND_FACTOR`), and `graphql_handler` gives an `X-API-Key` request no cookie jar (textual pin `api_key_cookie_jar_pin`). **Not changed**: keys' `IsTwoFactorVerified(true)`; the client-supplied enrolment secret. (2) Ten bcrypt hashes ran inline, before the already-enabled refusal; backup verifies ran inline under an advisory lock. Now throttle → enabled read → code → `spawn_blocking`; a backup code is spent by one conditional `array_remove` UPDATE (lock deleted). **bcrypt KEPT** (48-bit codes). `check_enrolment_throttle`: 10 per 900 s per user, setup + enable share it, Redis with in-memory fallback even in production (bounds cost, not guessing). (3) "Someone holds `automation-node`" meant done, so removing that grant elevated the next signup and restarts re-granted. Now `capability_bootstrap` (migration `20260925140000`) makes it one-shot; backfill needs a top grant or its `admin_event_log` record (older revokes name no world — such a deployment re-promotes once, stated). Self-revoke that widens (`withdrawing_grant_widens`: `minimal-node`, `governance-node`) is refused for everyone; admins revoke others'. Guard `controller/tests/two_factor_session_tests` (CTRL_TESTS), each test shown failing pre-fix. `--count` stays 97.
...
* **ET, 2026-09-25 — integration connect flows bind `state` to the browser; a GitHub App installation is claimed only with GitHub's proof of access and never moved off an active owner.** Two cross-tenant defects from the 2026-09-25 review. (1) A connect `state` proved which Talos user STARTED a flow, not which browser FINISHED it, so an authorize URL minted under an attacker's account and completed by a victim stored the victim's refresh token under the attacker (gmail, google-calendar, gcp ×3 tiers, slack, atlassian, GitHub). ONE home `talos_oauth::connect_binding`: `/connect` sets `talos_oauth_connect` (`HttpOnly`, `SameSite=Lax`, 600 s, `Secure` in production; REUSED when well-formed so two tabs do not invalidate each other; deliberately NOT the login flow's single-use cookie), the state row stores only its SHA-256, and `consume_oauth_state` REFUSES a missing, ambiguous, mismatched or ABSENT stored binding AFTER the burn. `begin_oauth_authorization` / `authorization_url` take `&BrowserBinding`, the callbacks `presented_binding` — by TYPE, not convention. **Deploy seam, stated**: connects in flight at deploy (≤10 min) fail once. (2) `/api/github/setup` trusted the spoofable `installation_id` and upserted `user_id = EXCLUDED.user_id`. Now setup writes NOTHING and starts GitHub user authorization (PKCE, second browser-bound state carrying the id in `oauth_state_tokens.bound_subject`, migration `20260925100000`); `/api/github/authorized` claims only on `InstallationAccess::Accessible` from `GET /user/installations`, through `GithubAppInstallationRepository::claim_recorded` (the ONE owner writer, `upsert` DELETED) whose conflict arm is `WHERE user_id = EXCLUDED.user_id OR NOT is_active` — 0 rows is `OwnedByAnotherUser` — with `github_installation_claimed` in `admin_event_log` in the same transaction; refusals are `talos_audit` `github_installation_claim_refused`. **Deliberately NOT GitHub's "Request user authorization during installation"** (the user-suggested mechanism): it bypasses the Setup URL and is reported to drop `state`; the explicit hop keeps our state + PKCE. New REQUIRED-for-connect config `GITHUB_APP_CLIENT_ID`/`_SECRET`/`_REDIRECT_URI` (all or none): unset ⇒ connect 503 + boot WARN, existing installations still mint. **Latent here**: dev has no user-auth credentials, so its GitHub connect is OFF until they are set. Six mutations, six caught. **Stated limits**: no metric on either refusal; the user token is not revoked after the check (zeroized, never stored); a user holding one GitHub-verified installation can still take an INACTIVE row of another user; no lint (population: one consume, one claim writer).

* **ET, 2026-09-25 — EQ's `fetch_all` limit closed at the TYPE level; and four candidates measured and NOT changed.** EQ recorded, verbatim and correctly: *"`fetch_all` iterates `&reqs`, so the original stays reachable and a reverted push is caught by the pin alone."* That sentence is left BYTE-IDENTICAL above (check 96 leg 1) and is superseded here. The loop now CONSUMES `reqs`: the body is MOVED into the validated tuple on both pass-through arms (empty body, and `Ok(None)` = no marker) instead of cloned, and the url with it. **Proven, not argued**: re-applying EQ's silent mutation — pushing the PRE-SUBSTITUTION body — now fails to build with `error[E0382]: borrow of moved value: req.body`, so the guarantee moved from a lint finding to a borrow-checker error on a credential-substitution path. The resolver's result is bound to a local FIRST, because a temporary in a `match` scrutinee lives to the end of the match and resolving inline would keep `&req.body` borrowed through the arms. **Coverage is better than EQ stated**: its two `fetch_all` guards are BEHAVIOURAL (they drive the real `fetch_all` through the WIT Host trait), so this path has a test, a both-surfaces pin, and now the compiler. **The performance half is LATENT on this fleet and that is stated rather than implied**: one clone of every request body (cap 10 MB/entry, multiplied by `buffer_unordered` concurrency — the loop's own comment flagged the double-clone) plus one url clone are removed, but `fetch_all` has one live caller and it carries no marker, so nothing measurable changes here today. **Four candidates measured and NOT changed, so the next session does not redo them.** (1) The ES ack/defer class does NOT extend to the webhook router — it already answers failures with 15x `INTERNAL_SERVER_ERROR` and 3x `SERVICE_UNAVAILABLE` against 8x `OK`. (2) `execution_events.error_class` is NULL on all **133 944** rows and that is CORRECT-BY-CLASSIFICATION, with the mechanism now known: the dispatcher's non-transient branch emits BOTH producers — the `retry_skipped` event carrying `error_class` and the `"Job failed (non-transient: <class>)"` wrapper `extract_non_transient_class` parses back — and that branch needs non-transient AND no `retry_condition`, which has never co-occurred (`retry_skipped` = 0 rows against 75 `node_failed` / 148 `node_retrying`). EM's "latent, not dead" stands; do not re-open it. (3) **A JWK boot-prewarm was investigated and REJECTED on the denominator**: `GoogleOidcVerifier::new()` does start with an empty key map and fetch lazily, and this deploy refused 14 pushes across two 60 s backoff windows — but 7 days hold **63 controller restarts against only 3 fetch failures**, so the boot fetch almost always succeeds and the 201 `unknown_key` refusals are those 3 windows' amplification, exactly as AP designed (one WARN per window, every push redelivered). (4) The catalog seeder's `UPDATE modules SET .. source_code = ..` re-upsert (225 calls in a 10-minute window, 115 rows averaging 130 kB) is **absorbed**: 219 of 255 updates were HOT, 3 dead tuples, autovacuum current, table 16 MB — package X's same-value-write class does not bite here. **Incidental live confirmation of ES's premise**: the 14 pushes refused 401 during this deploy's backoff were REDELIVERED by Pub/Sub and processed — the first `pa-ask-email` success is the same second as the window-close line — so a non-2xx really is retried on this fleet, which is what ES's 503 choice rests on. **`--count` stays 97**; no new test binary.
* **ET, 2026-09-25 — four privilege-ceiling / egress gaps from a 2026-09-25 review.** **(1) The verb axis was dropped wherever ceilings are COPIED**: `apply_subworkflow_binding` stamped tier/write/egress and never `http_verb_ceiling`, `get_workflow_actor_binding` never read the column and the resolver answered `None` (DB-error arm too), so a child bound to a `readonly` actor inherited a parent's `Some(Write)`; the clone copied three of four columns. ONE value `talos_workflow_engine_core::ActorCeilings` (four signed axes, no `Default`, `FAIL_CLOSED` explicit on every axis) stamped by the exhaustively-destructuring `ParallelWorkflowEngine::set_ceilings` from `apply_actor_to_engine`, the sub-workflow binding and its DB-error arm; narrowed by `ActorCeilings::narrowed_for_child` on EFFECTIVE values (a child's NULL override inherits ITS OWN ceiling); `ActorCeilingColumns` + the clone INSERT destructure exhaustively. **CORRECTS EL**: `narrow_verb_inference_override` DELETED — it read a child's `None` as "no opinion" and its own test pinned `(Some(Write), None) → Some(Write)`. `max_capability_world` deliberately NOT a member (its `None` means exempt; not on the wire). **(2) automation-node `wasi:http`** linked upstream's outgoing-handler over `default_hooks()` (raw connect, no gate) and JS `fetch` compiles to it; the "operator-authored" premise was false (any `automation-node` grantee compiles one). Now `talos-worker-runtime/src/host/wasi_http.rs::gated_handle`, registered with `func_wrap` on `wasi:http/outgoing-handler@0.2.12` (the generated trait has `types::Host` as a supertrait), applies `fetch`'s gate set through the pure `wasi_http_admission`, records refusals synchronously (`record_capability_denied_now`: ledger `try_lock`, so the event precedes the terminal anchor), and `HardenedWasiHttpHooks` sends through the execution's hardened client (SSRF + local-only egress at connect, no proxy/redirects, response cap). Refusals use the `http-fetch` op label (target `wasi:http <host>`), so the ceiling's op partition is unchanged; `ceiling_axis_pins` scans the site and gains `every_op_has_exactly_one_axis_across_all_its_sites`. An authority that does not round-trip (`:99999`) is refused. Stated limits: linker wiring pinned TEXTUALLY (no component-level test); a resolver refusal reaches the guest as `ConnectionRefused` with no ledger row; no `vault://` substitution; `HEAD`/`OPTIONS`/`CONNECT`/`TRACE` are never declarable, so always refused. Latent here: 1 automation-node module, empty `allowed_hosts`. **(3) `create_workflow_from_spec`** compiles inline nodes through `InlineCompileService`, split into `compile_checked` + `persist_compiled` (`CompiledInline` bound to name, world and source hash); ids, inline grants and edges validate and every node compiles BEFORE any write; `NameCollision::Refuse` refuses a taken name pre-compile and again at persist; hosts are explicit per node and never `"*"` (default none); the handler runs `require_agent_role_permits_world` over `inline_compile_worlds` (same resolution precedence); `module_id` nodes are visibility-gated; duplicate ids refused. `update_node_template_wasm` / `insert_node_template` DELETED (UPDATE with no owner predicate). **Behaviour change, stated**: `InlineCompileService` persisted `allowed_methods` as a literal `&[]`, so `add_node_to_workflow`'s explicit methods were dropped — now written. Stated limit: a persist-phase failure can leave earlier nodes' modules. **(4) tier 2 + `egress_scope = local`** reached public IP literals (the deny sat inside `if Tier1`; a resolver never sees a literal): `egress_posture_deny_reason` is the one predicate at five surfaces plus `wasi:http`, policy `local-egress-public-ip`, class `tier1-egress` (closed set unchanged). Mutation pass PARTIAL: M1 (`set_ceilings` drops the axis) and M2 (raw-`Option` narrowing) caught, the one-axis flip caught; the full 14-mutation run was stopped at M3, which was left in the tree and restored — the harness's `finally` does not survive a kill. No lint added, `--count` stays 97.
* **EU, 2026-09-25 — four execution paths repeated side effects or skipped actor gates.** (1) The worker's NATS path passed a blind three-retry `RetryPolicy::default()` on top of the controller's method-aware re-dispatch: a POST node allowed 0 retries ran up to 4×, a GET node allowed 2 up to 12×. `RetryPolicy` has no `Default` now; `controller_dispatched()` is the NATS answer, pinned by `worker/src/retry_policy_pin.rs`; `in_process_transient()` names the old policy for the six embedded callers, behaviour unchanged. (2) `enqueue_workflow` discarded its `queued→running` claim's `bool` and ran rows cancelled while queued: `EnqueueClaim {Claimed, NotQueued, Unreadable}` + `drain_enqueued_batch` dispatch only `Claimed`; `Unreadable` fails closed and finalizes via `fail_execution_unless_terminal`. `#[must_use]` (with a reason) on the five `Result<bool>` status transitions — stated: it cannot see `if let Err(e) = …`, the enum is the guard. (3) The same drain built `EngineOpts::for_run` with NO actor, and the gate ran only for a caller-named actor: the actor now resolves once via `resolve_effective_actor(arg.or(workflow actor))` above admission, stamps the rows and binds every engine. `for_run`/`for_skip_load` return `UnboundEngineOpts`, which `for_workflow` rejects until `with_effective_actor` / `with_actor_id` / an explicit `without_actor()`; check 56 matches `without_actor()` and its roots are every crate's `src/` (the crate list missed two engine builders), at zero. (4) Loop bodies skipped the ceiling, approval gate and rate limit and sent `max_retries: 2`: `clear_module_dispatch` is the one home shared with single-node dispatch, `check_rate_limit` runs per iteration, new `termination_reason`s `dispatch_refused` / `rate_limited`. **Deliberately NOT**: folding the rate limit into the shared method (single-node treats it as a soft `__error`, not a failure); loop-body idempotency (one key would dedupe iterations 2..N). **Stated limits**: no test observes in-process attempts (a WAT guest has no side channel), so item 1's guard is the textual pin plus the constructor test; the pipeline path keeps its own inline gates. `--count` stays 97.

* **EU, 2026-09-25 — three workflow-engine correctness defects from the 2026-09-25 review.** (1) **Edge conditions and edge types applied only after a worker-dispatched module**: every other commit (13 `commit_result!` + `unblock_successors` pairs and `route_system_node_output`'s success branch) only decremented counters, so a judge `passthrough` ran BOTH conditional branches and a system node's error handler ran on success. ONE path `release_successors(idx, Release::{Succeeded, SkippedItself, FailedToErrorEdges, ContinuedAfterFailure})`; `unblock_successors` / `unblock_child_on_failure` / `apply_fan_in_early_ready` DELETED; `commit_and_release!` is the only inline commit (pauses use `commit_paused_result!`). Skips are a pure join table (`join_state::Joins`): a node runs once EVERY incoming edge resolved and ≥1 resolved active, else it is recorded `{"__skipped": true, "reason": "no_active_input"}` and the skip cascades to the bottom — DECIDED OR-join, order-independent; early-ready fan-in counts ACTIVE parents and a `continue_on_error` failure never satisfies one alone. **Stated behaviour changes**: a conditional edge out of a self-skipped node (`skip_condition`, unmatched `error_handler`) is inactive WITHOUT evaluation; a rate-limited node releases as a continued failure (its historical all-edges shape); Loop / CapabilityDispatch fail-fast unchanged (`BranchLocal`, recorded). **Stated limit**: a resume seeds a completed parent's edges active without re-evaluating conditions. (2) **Cancel did not stop the engine** — the token was fired only by the epoch fence, while the reply said "No further nodes will be dispatched". `InFlightRuns::cancel` fires a per-run stop signal that all three `nats_run` entry points race (`run_tracked`, textual pin = 3); `cancel_execution` fires it only AFTER the ownership-checked UPDATE and reports `EngineStop::{StoppedHere, NotRunningHere, NotAttempted}`; the fence reads `(epoch, status)` and stops on a terminal status (cross-replica backstop, fenced paths, ≤10 s). **Breaking**: `ModuleExecutionStore::record_started -> Result<StartedRow, _>`; `BornCancelled` → the engine sends nothing, fires the per-run abort token on `ExecutionProgress`, routes no completion after it, and answers `Cancelled`. Reply text from `cancel_reply_message`. **Stated limits**: an unfenced run on ANOTHER replica stops only at its next top-level module dispatch; sub-workflow children run under a synthetic execution id, so their start rows never see the parent's status. (3) **Byte-slice panics** in the ensemble majority-vote key and the MCP waterfall label; one engine helper `validation::truncate_at_char_boundary`. **`clippy::string_slice` measured and NOT enabled**: 286 sites, 13 with a fixed byte cap, all ASCII by construction — nothing the fixes miss. Guards: `tests/edge_routing.rs`, `tests/ensemble_vote_key.rs`, `tests/cancellation.rs`, `controller/tests/execution_cancel_engine_stop_tests` (CTRL_TESTS); 12 mutations applied, 12 caught. `--count` stays 97.
* **EU, 2026-09-25 — four silent data-loss paths closed.** (1) **Editor save deleted MCP-authored controls**: the loader read only `id/type/position/data` and the saver spread EMPTY typed fields after the config. ONE home `frontend/src/lib/graphDocument.ts`: controls read with the ENGINE's precedence (skip/timeout `data` first; `retry_*` top level first; continue-on-error OR), stripped from config, written back only when defined; unknown node/edge/graph keys (`kind`, `description`, edge `logic`, `execution_timeout_secs`) carried verbatim; an ill-typed control round-trips untouched; `RetryPolicy.maxRetries` optional (absent ≠ 0; the inspector no longer invents 0). (2) **`graph_json` lost updates**: migration `20260925120000` adds `workflows.graph_version`, OWNED BY A TRIGGER (`trg_workflows_graph_version`, bumps when `graph_json` changes on ANY writer; explicit values overwritten). ONE home `talos_workflow_repository::graph_version`; `update_workflow_graph` REQUIRES the expected version and returns `#[must_use] GraphWrite { Written, Conflict, NotFound }` — a foreign row is `NotFound`, never `Conflict` (no existence oracle). All MCP read-modify-writes, `set_workflow_priority`, auto-fill and the failure-analysis auto-fix (a second unconditional copy, DELETED) use it; GraphQL `updateWorkflow(expectedGraphVersion:)` refuses with `extensions.code = GRAPH_VERSION_CONFLICT`, and the editor sends it. **Conflict is returned, not auto-retried** (re-applying needs the handler's mutation; a JSON merge was rejected). Deliberately unconditional, version still bumped: rollback, platform-state import, GraphQL update without a version. (3) **Consolidation retired unsummarised rows**: `plan_consolidation` packs WHOLE rows and retires exactly those; `consolidate_memory_guarded` is compare-and-delete on each row's read `updated_at`; all-changed rolls back. (4) **Bulk module cleanup deleted bound modules + history**: survey, `cleanup_modules` and hygiene `fix_all` exclude webhook-bound, push-bound (`PushBoundModules`, required; unreadable ⇒ REFUSE) and history-bearing modules. **The CASCADE is KEPT**: `module_executions.module_id` is NOT NULL and read by ~70 files, RESTRICT was the Phase-5 regression; explicit `delete_module` still cascades (stated). Tests: `graph_version_tests` (deterministic MCP race via row lock + `pg_stat_activity`), `workflowRoundTrip.test.ts`, consolidation unit + `talos-memory` DB tests, `preview_action_scope`; every fix mutation-proved. No lint added; `--count` stays 97.

* **EV, 2026-09-25 — six guest-reachable resource-exhaustion paths bounded.** (1) **Compile spawns** were `timeout(d, cmd.output())` with no `kill_on_drop`: the permit dropped while the `podman`/`docker run` container kept running (reproduced). ONE home `talos_compilation::sandbox_run`: builders return `SandboxCommand` (private `Command`; only `run(deadline, &CompileSlot)`, so `.output()` does not compile); containers `--name talos-sandbox-<uuid> --label talos.sandbox=1`, `rm -f`'d before `TimedOut` returns; host children own a process group, SIGKILLed on timeout, cancel and normal exit; cancel = reaper THREAD holding a `CompileSlot` (`Arc<OwnedSemaphorePermit>`) clone until `rm` ends; output capped 8 MiB/stream; `analyze_code` now takes a slot. (2) **Controller WASM wall clock**: only the worker started an epoch ticker. `TalosRuntime`'s constructor starts `EpochTicker` on an OS THREAD (`EngineWeak`, stop-on-drop); `spawn_epoch_ticker` DELETED (a second ticker halves budgets); granted slices `Yield`; cache-miss compiles `spawn_blocking`. **REFUTED**: `memory.fill/copy` are NOT flat-5-fuel on wasmtime 47 (`pre_translate_bulk_op` charges the byte length; a 16 MiB fill loop exhausts 10M fuel in 40 ms) — the hole is HOST work at a few fuel per call (a `json::parse` loop ran past a 30 s watchdog with no ticker, 2.2 s with one). (3) **Host fns**: GraphQL fragment scan linear (`match_braces`; the retained quadratic reference took 90 s on a 1 MB flood) and moved after the allowlist + rate charge; `MAX_GRAPHQL_RETRIES_PER_CALL` = 5; `json_to_xml`/`csv_to_json`/`json_to_csv` write through a capped `BoundedBuf` (bytes and boundary unchanged); `SseParser` reads by cursor and queued events hold a 4 MiB/stream permit budget; `fetch_all` shares one per-call response budget (`max(32 MiB, per-entry cap)`). (4) **Breaker poisoning**: guest timeouts floored at 1 s (`clamp_guest_timeout_ms`); breaker keyed `host:port` (`BreakerTarget`); a timeout under 10 s, a builder error or any non-default port is no evidence (`send_error_is_host_evidence` via `settle_send_error`). Webhook IP breaker keyed `(ip, trigger_id)` (cost: 10 attempts per known trigger); MCP `reset_webhook_circuit_breaker` had called the no-op `record_success` and answered `"cleared": true` — `reset_ip` now reports what it removed. (5) **Guest SQL**: `DISALLOWED_SQL_FUNCTION_PREFIXES` (`pg_advisory_`, `pg_try_advisory_`, `lo_`; exact `loread`/`lowrite`) in the one matcher both fences call; `execute_guest_query` owns its connection in `GuestSession` OUTSIDE the budget, `release()` (`pg_advisory_unlock_all`) is the only way back to the pool, every other exit detaches (NOT `DISCARD ALL`); migration `20260925160000` revokes PUBLIC and re-grants CURRENT_USER + `talos_app` (required: `upsert_secret` locks as `talos_app`) only where PUBLIC had EXECUTE (never widens `lo_import`). A literal `1/0` folds at plan time, before the lock. (6) **Memory**: at most `GRAPH_EXTRACTION_MAX_PENDING` = 64 extraction tasks (overflow sheds, `graph_extraction_shed_total()`); `MAX_MEMORIES_PER_ACTOR` enforced in `PERSIST_MEMORY_ROW_SQL` for both persist paths (existing key OR all rows incl. expired < cap; reclaim expired once; `MemoryWriteError::QuotaExceeded`, label `quota` pre-seeded; the memory RPC answers `StorageFull` by type); `compress_actor_context` retires before writing. **Deliberately NOT**: a guest SQL function allowlist (0 guest statements measurable), an exact memory cap (READ COMMITTED overshoot ≤ concurrent writers), `clone_memories`, podman `--timeout`, per-byte bulk fuel. **Stated limits**: a SIGKILLed controller orphans its container (the label finds it); host `setsid()` escapes; the REVOKE is a no-op on managed Postgres and binds only with `TALOS_RPC_GUEST_ROLE`; env-based tests assume nextest's process-per-test (`make test`, CI) — in-process `cargo test` of `talos-worker-runtime --lib` is flaky, as `fetch_with_bearer` already was. Every fix mutation-proved; two first-draft tests passed vacuously and were rebuilt (a fuel-exhausting fill loop; a time bound an unbounded memcpy met). No lint added; `--count` stays 97.

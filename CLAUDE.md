...
**Deliberately NOT done / recorded.** Separate worker NATS user (G4c); a post-auth DLQ enqueue site; TOTP/OTLP AAD domain separation (needs a reader fallback + format bump); the three v3 writers on org data; `set_actor_llm_tier_ceiling` loosening on a no-2FA credential (product call); webhook POST retry default 3 (pinned contract); Unicode look-alike delimiters; `frontend/src/generated/*` not regenerated (no `node_modules` here — run `npm run codegen`; the only SDL change is the optional `disableTwoFactor(code)` arg).

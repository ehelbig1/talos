...
pub use talos_memory::{
    ...
    ForgetOutcome, ForgetPrefixOutcome, MemoryHit, MemoryMeta, MemoryRow, MemoryWriteError,
    PersistOutcome, SearchMethod, SearchOutcome, MAX_LIST_LIMIT, MAX_MEMORIES_PER_ACTOR,
...
/// The inbound reserved-key strip. ONE home:
/// [`talos_workflow_engine_core::reserved_keys::strip_engine_authored_keys`] —
/// the engine applies the same function at its own trigger-seed install and on
/// every committed module output, so the controller seam here and the engine
/// cannot drift on WHICH keys are engine-authored. Re-exported so this crate's
/// callers keep compiling; the key list itself is
/// `reserved_keys::ENGINE_AUTHORED_INPUT_KEYS`.
pub use talos_workflow_engine_core::reserved_keys::strip_engine_authored_keys;

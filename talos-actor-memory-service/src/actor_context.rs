...
pub async fn get_recent_actor_context(
    ...
    talos_memory::recall_recent_by_types(repo.pool(), actor_id, &["working", "episodic"], 10).await
}
...
pub async fn get_relevant_actor_context(
    ...
    let rows =
        get_relevant_actor_context_unscoped(repo, actor_id, limit, context_hint, execution_id)
            .await?;
...
async fn get_relevant_actor_context_unscoped(
    ...
    if talos_config::smart_memory_context_enabled() {
        return get_relevant_actor_context_smart(repo, actor_id, limit, context_hint, execution_id)
            .await;
    }
            ...
            match graph.get_graph_context(actor_id, hint, 2, 20).await {
                Ok(ctx) => {
                    let entity_count = ctx
                        ...
                        .unwrap_or(0);
                    if entity_count > 0 {
                        results.push(("__graph_context__".to_string(), ctx, "graph".to_string()));
                    }
                }
                Err(e) => {
...
async fn get_relevant_actor_context_smart(
    ...
    let (weights, access_weight) = if talos_config::adaptive_rank_enabled() {
        ...
        talos_memory_ranking::load_serving_weights(repo.pool(), actor_id)
            .await
    ...
    let feature_snapshots: std::collections::HashMap<String, (f64, f64, f64, Option<f64>, f64)> =
...
pub async fn get_recent_actor_context_limited(
    ...
    get_relevant_actor_context(repo, actor_id, limit, None, None, MemoryScope::Full).await
}
...
pub async fn upsert_scratchpad_trace(
    ...
    talos_memory::persist_memory(repo.pool(), actor_id, key, value, "scratchpad", None).await?;
    Ok(())
}

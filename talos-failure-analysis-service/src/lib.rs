...
pub fn remediation_steps(error_type: &str, module_label: &str) -> Vec<serde_json::Value> {
    match error_type {
        ...
        "write_ceiling_denied" => vec![
            ...
            serde_json::json!({ "step": 3, "action": "decide_deliberately", "description": "Either make the call read-only (usually the right answer for a reporting workflow), or grant deliberately. If the refused call is a POST that is a READ on the target API (Plaid, GraphQL, most client_id+secret APIs), the NARROW grant is set_actor_http_verb_ceiling with 'write' — it lifts the verb inference on the three inferring gates and leaves the twelve categorical ops (agent-memory, database, email, messaging, webhook, object-storage, integration-state) governed by max_write_ceiling. Raise max_write_ceiling with set_actor_write_ceiling only if the mutation is genuinely intended, since it grants all fifteen. For the strict-egress case, naming the host explicitly in allowed_hosts instead of matching it by wildcard also resolves it.", "tool": "set_actor_http_verb_ceiling" }),
        ...
        "method_not_allowed" => vec![
            ...
            serde_json::json!({ "step": 3, "action": "declare_the_method", "description": "Add the verb with update_module_methods. Declare the verbs you actually use — clearing the list does NOT grant everything: since 2026-09-24 an EMPTY allowed_methods DENIES every verb, so an empty list is what produced this refusal if the module declared nothing. There is no wildcard; 'every verb' is [\"GET\",\"POST\",\"PUT\",\"PATCH\",\"DELETE\"] written out. An empty list also forfeits the automatic read-only retry default, so new nodes from that module get retry_count 0.", "tool": "update_module_methods" }),

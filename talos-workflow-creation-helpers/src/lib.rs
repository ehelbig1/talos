...
/// Validate config field types against a JSON-schema-shaped `properties`
/// declaration on a module template.
///
/// Only top-level fields are checked (matching the historical inline
/// behaviour). For each field the caller passed, we look up the declared
/// `type` in `config_schema.properties.<field>.type` and compare it against
...
///   * `string` / unspecified → accept as-is (matches historical lenient mode)
///
...
pub fn validate_config_field_types(
    ...
    for (field, value) in config_obj {
        let Some(prop_def) = props.get(field) else {
            continue;
        };
        let expected = prop_def.get("type").and_then(|t| t.as_str());
    ...
    if errors.is_empty() {
        ...
        Err(format!(
            "Config type mismatch — correct before adding node \
             (avoids Wasm runtime errors and wasted retries):\n{}",
            errors.join("\n")
        ))
    }
}
...
mod tests {
    ...
    #[test]
    fn config_type_validation_passes_correct_types() {
        let schema = json!({
            "properties": {
                "count": {"type": "integer"},
                "active": {"type": "boolean"},
        ...
        assert!(validate_config_field_types(&cfg, &schema).is_ok());
    }

    #[test]
    fn config_type_validation_fails_on_mismatch() {
        let schema = json!({"properties": {"count": {"type": "integer"}}});
        let cfg = json!({"count": "not a number"});
        let err = validate_config_field_types(&cfg, &schema).unwrap_err();
        assert!(err.contains("'count'"));
    ...
    #[test]
    fn config_type_validation_lenient_for_string_or_unspecified() {
        let schema = json!({"properties": {
            "name": {"type": "string"},
            "open": {}, // no type — accept anything
        }});
        // String with a number stored as string — schema says string,
        // value is "5", that's a string, accepted.
        let cfg = json!({"name": "5", "open": 42});
        assert!(validate_config_field_types(&cfg, &schema).is_ok());
    }

    #[test]
    fn config_type_validation_skips_unknown_fields() {
        // Field present in config but not declared in schema — the
        // historical inline behaviour ignored it (no extra-property
        // strictness). Preserve that.
        let schema = json!({"properties": {"declared": {"type": "string"}}});
        let cfg = json!({"declared": "ok", "undeclared": [1, 2, 3]});
        assert!(validate_config_field_types(&cfg, &schema).is_ok());
    }

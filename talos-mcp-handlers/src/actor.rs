    ...
    vec![
        ...
        serde_json::json!({
            ...
            "description": "Set the data-mutation (WRITE) ceiling for an actor — a default-deny gate on data-mutating operations. \
                ...
                WHAT 'MUTATING' MEANS, and where it is INFERRED rather than proven: the HTTP legs classify by VERB — GET is the only read verb — and the GraphQL leg treats every call as a mutation because the operation type is not provable from the request string. Both are the right fail-closed default and both mean a READ-ONLY integration can still need 'write': an API whose reads are POSTs (Plaid, GraphQL, most client_id+secret APIs, many search endpoints) is refused under 'readonly'. Since 2026-09-24 the ceiling CAN express 'may POST, but only reads', but not on this axis: set the actor's http_verb_ceiling to 'write' with set_actor_http_verb_ceiling, which lifts the verb inference on exactly the three inferring gates and leaves THIS ceiling governing the twelve categorical ops (agent-memory, database, email, messaging, webhook, object-storage, integration-state). Raising this ceiling to 'write' also clears the refusal and grants all fifteen — more than a POST-shaped read needs. The controls that bound such a module either way are its allowed_methods, its allowed_hosts and its source. Only the SQL leg proves it, by walking the statement's AST. NEW actors default to 'readonly' (so a freshly-built workflow can't silently mutate your data); existing actors were grandfathered to 'write'. Grant 'write' deliberately to actors that need to mutate.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "actor_id": { "type": "string" },
...
/// Thin wrapper over `ActorLifecycleService::scaffold` (which parses
/// MCP args into the typed request and dispatches to
/// `talos_actor_scaffold::scaffold_actor`). Maps errors to the correct
/// JSON-RPC code (-32602 for input validation, -32603 for capability
/// ceiling, -32000 for DB errors).
async fn handle_scaffold_actor(
    ...
    // Arg parsing/validation (the MCP-376/457/280/304/382/1183/276/1224/
    // 345/350/255/348 boundary stack) and the scaffold orchestration both
    // live in `talos-actor-lifecycle-service`; error strings and the
    // -32602/-32603/-32000 code mapping are preserved byte-for-byte.
    match state.actor_lifecycle_service.scaffold(user_id, args).await {
        Ok(outcome) => mcp_text(
            req_id,
...
// The budget-precheck family (ActorBudget, load_actor_budget,
// check_actor_status, the hour/total budget gates, and
// check_execution_allowed[_for_batch]) moved whole-cloth to
// `talos_actor_repository::budget_precheck` (2026-07-02) so the
// actor-lifecycle service can enforce the same gates on the handoff
// dispatch path without depending on this handler crate. Re-exported
// here so legacy `crate::actor::*` callers keep resolving.
#[allow(unused_imports)]
pub use talos_actor_repository::budget_precheck::{
    check_actor_hour_budget, check_actor_hour_budget_for_batch, check_actor_status,
    check_actor_total_budget, check_actor_total_budget_for_batch, check_execution_allowed,
    check_execution_allowed_for_batch, load_actor_budget, ActorBudget,
};
...
async fn handle_handoff_to_actor(
    ...
    // The full gate sequence (arg parsing interleaved with actor-status
    // checks, chain cycle/depth safety, from-actor budget precheck,
    // to-actor `authorize_workflow_trigger` — MCP-727, execution insert,
    // dual audit logs, background engine dispatch) lives in
    // `talos-actor-lifecycle-service::ActorLifecycleService::handoff`.
    // Error strings, first-failing-check ordering, and the response body
    // are preserved byte-for-byte from the pre-extraction handler.
    match state.actor_lifecycle_service.handoff(user_id, args).await {
        Ok(outcome) => mcp_text(
            req_id,
            &serde_json::to_string_pretty(&outcome.to_tool_body()).unwrap_or_default(),
        ),

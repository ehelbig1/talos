...
pub fn tool_schemas() -> Vec<serde_json::Value> {
    vec![
        ...
        serde_json::json!({
            ...
            "description": "Diagnose why a failed workflow execution failed. Fetches failed-node events, classifies error type, and returns numbered remediation steps. Call this immediately after an execution fails to get actionable next steps.\n\nBUCKETS. Prose-matched (from the module's own error text): output_schema_violation / module_compile_error / json_parse / missing_secret / rate_limit / fuel_exhausted / wasm_trap / timeout / network_error / config_error / http_401 / http_403 / http_404 / http_5xx / auth_error / database_error, plus the fall-through `unclassified`. Host-stamped (from the worker's `[reason_class=<token>]` marker, which names the gate that actually refused an outbound call): host_not_allowed / capability_world_denied / ssrf_blocked / egress_tier_denied / write_ceiling_denied / method_not_allowed / insecure_scheme / invalid_url / request_too_large / response_too_large / egress_budget_exceeded / introspection_denied / circuit_open / execution_cancelled. When a marker is present the report also carries the raw `reason_class` token per failed node. `circuit_open` is additionally matched WITHOUT a marker, because the breaker's retry-gate fast-fail is plain prose: the same refusal used to answer two different buckets depending on which of the two breaker paths fired.\n\nSCOPE LIMIT, and it is narrower than it used to be. A host-policy denial is DECIDED in the worker and its full detail — the policy name and the TARGET — is written only to `module_execution_logs`, not to the engine event stream this reads. Since the worker started stamping `[reason_class=…]` onto the node-failure message, the FAMILY of the denial does reach here and is classified above; the precise variant and the target still do not, which is why every denial playbook's first step is tail_worker_logs(execution_id). An UNMARKED failure (a pre-marker row, or a message so long the marker fell past the 4 KiB classification cap) still arrives as whatever the module returned afterwards — commonly a bare `invalidurl` that classifies as `unclassified`. `unclassified` means the cause is UNKNOWN and, in particular, that it is NOT established that the fault is in the module: read raw_error and tail_worker_logs before changing anything. It is not a diagnosis, and it replaced a fall-through that used to answer `runtime_error` / \"An unexpected runtime error occurred inside the module\" for exactly this case. When apply_fix=true and a config_error is detected with an extractable field, automatically patches the workflow graph.",
            "inputSchema": {
                "type": "object",
                "properties": {

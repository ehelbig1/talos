...
/// Same as build_node_label_map but also extracts the display label from node.data.label.
fn build_node_display_label_map(
    graph_str: Option<String>,
) -> std::collections::HashMap<uuid::Uuid, String> {
    let mut map = std::collections::HashMap::new();
    if let Some(gj) = graph_str {
...
async fn handle_analyze_execution_failure(
    ...
    if status != "failed" && status != "cancelled" {
        return mcp_error(
            ...
            -32000,
            &format!(
                "Execution status is '{}' — only failed or cancelled executions can be analyzed.",
                status
            ),
        );
    }
    ...
    let all_events = match state.execution_repo.list_execution_events(exec_id).await {
        Ok(evs) => evs,
        Err(e) => {
            tracing::error!("analyze_execution_failure events fetch failed: {}", e);
            return mcp_error(req_id, -32000, "Database error fetching execution events");
        }
    };
    ...
    fn truncate_for_classify(s: &str) -> &str {
    ...
    fn remediation_steps(error_type: &str, module_label: &str) -> Vec<serde_json::Value> {
        match error_type {
    ...
    // Determine if auto-fix is available: config_error with extractable field name
    fn extract_config_field(raw_error: &str) -> Option<String> {
    ...
    let apply_fix_candidate = failed_nodes.iter().find_map(|n| {
        if n.get("error_type").and_then(|v| v.as_str()) == Some("config_error") {
            ...
            let node_id = crate::utils::json_optional_string(n, "node_id");
            let node_label = crate::utils::json_optional_string(n, "label");
    ...
    fn extract_secret_name_from_auth_error(msg: &str) -> Option<String> {
        // MCP-1138: cap input before to_lowercase + repeated .find scans.

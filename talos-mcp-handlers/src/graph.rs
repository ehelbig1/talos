...
pub async fn dispatch(
    ...
    match name {
        ...
        "add_operator_digest_node" => {
            Some(handle_add_operator_digest_node(req_id, args, state, agent).await)
        }
...
async fn handle_add_operator_digest_node(
    req_id: Option<serde_json::Value>,
    args: &serde_json::Value,
    state: &McpState,
    agent: Arc<auth::AgentIdentity>,
    ...
    mcp_text(
        ...
        &serde_json::to_string_pretty(&serde_json::json!({
            "workflow_id": added.workflow_id.to_string(),
            "node_id": added.node_id,
            "node_type": "operator_digest",
            "days": days,
            "downstream": connect_to,
            "message": format!(
                "Operator-digest node '{}' added to workflow {}. Emits {{available, window_days, generated_at, ran, learned, needs_me, cost}} — the autonomy-cockpit feed for downstream compose nodes.{}",
                added.node_id, added.workflow_id, added.auto_publish_note
            ),
        }))
        .unwrap_or_default(),

...
// `SlackRateLimiter` (the entire struct + its `new`/`allow`/
// `cleanup_old_entries` methods) is a 90-line Slack-specific
// rate-limiter implementation with per-channel and global windows,
// but it has NO callers. Active Slack-event rate-limiting flows
// through the worker's per-execution rate-limit + the generic
// talos-rate-limit middleware. This implementation is vestigial,
// kept for a future "Slack-events-ingestion-per-channel" use case
// that hasn't materialized. Removing the attribute would surface
// the warnings; properly removing the dead code is out of scope
// for the documentation-style sweep — that's a follow-up.
#![allow(dead_code)]

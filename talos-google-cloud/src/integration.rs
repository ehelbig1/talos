...
impl GoogleCloudIntegrationService {
    ...
    pub async fn get_access_token(&self, user_id: Uuid, provider_key: Uuid) -> Result<String> {
        let access_token_path = format!(
            "oauth/google_cloud/{}/{}/access_token",
...
impl talos_oauth::OAuthIntegration for GoogleCloudIntegrationService {
    ...
    async fn complete_callback(
        ...
        let scope = token_data.scope.unwrap_or_else(|| {
            "https://www.googleapis.com/auth/cloud-platform.read-only,\
        ...
        // Best-effort audit (shared channel-lifecycle log).
        self.audit(
            Some(integration.id),
            user_id,
            "google_cloud_connected",
...
impl GoogleCloudIntegrationService {
    ...
    pub async fn get_user_integrations(
        ...
        let integrations = sqlx::query_as::<_, GoogleCloudIntegrationInfo>(
            r#"
            SELECT id, account_email, account_name, scope, is_active,
    ...
    pub async fn disconnect_integration(&self, integration_id: Uuid, user_id: Uuid) -> Result<()> {
        // Step 1: recover provider_key (+ email for the audit label).
        let row: Option<(Uuid, Option<String>)> = sqlx::query_as(
            "SELECT provider_key, account_email FROM google_cloud_integrations \
             WHERE id = $1 AND user_id = $2 AND is_active = TRUE",
        )
        .bind(integration_id)
        .bind(user_id)
        ...
        let (provider_key, account_email) = match row {
            Some((pk, email)) => (Some(pk), email),
            None => (None, None),
        };

        // Step 2: best-effort provider revoke + vault cleanup.
        if let (Some(pk), Some(cred_svc)) = (provider_key, &self.credentials_service) {
            if let Err(e) = cred_svc
                .revoke_and_cleanup(user_id, "google_cloud", &pk.to_string())
                .await
            {
                tracing::warn!(
                    user_id = %user_id,

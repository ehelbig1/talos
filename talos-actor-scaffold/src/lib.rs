...
pub async fn scaffold_actor(
    ...
    if let Some(b) = &req.budget {
        ...
        // Stored defaults matching handle_set_actor_budget. max_workflows_per_minute
        // IS enforced (atomic per-actor trigger-rate cap in
        // WorkflowRepository::create_execution_under_concurrency_limit);
        // max_compilations_per_hour is still RESERVED (stored, not yet enforced).

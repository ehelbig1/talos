//! Capture git state at compile time so the worker's boot-time
//! self-registration reports the build it is ACTUALLY running, not just the
//! manually-bumped `Cargo.toml` version. Operators get a unique signature per
//! rebuild, and the controller can WARN when the fleet is split across builds.
//!
//! Deliberately a byte-for-byte copy of `controller/build.rs` /
//! `talos-mcp-handlers/build.rs` below this header: the two sides of the
//! build-identity handshake must derive their SHA the SAME way, or a
//! same-tree controller and worker would report different strings and the
//! skew WARN would fire on a healthy fleet. A shared build-dep crate is not
//! worth it for ~80 lines with zero runtime surface; keeping the three in
//! lockstep is a review item when any of them changes.
//!

...
def main():
    ...
    if 'allowed_hosts' in manifest:
        if len(manifest['allowed_hosts']) > 50:
            print("❌ Error: 'allowed_hosts' exceeds maximum of 50 entries.")
            sys.exit(1)
        for host in manifest['allowed_hosts']:
            if not host or not isinstance(host, str):
                print("❌ Error: 'allowed_hosts' entries must be non-empty strings.")
                sys.exit(1)
            if host == "*":
                continue
    ...
    # M-8 fix: controller now requires Authorization: Bearer $REGISTRY_PUBLISH_TOKEN
    # in production. Forward whichever of these env vars is set; in dev
    # the controller skips the check when REGISTRY_PUBLISH_TOKEN is unset
    # on the server side.
    publish_token = os.getenv("REGISTRY_PUBLISH_TOKEN") or os.getenv("TALOS_REGISTRY_PUBLISH_TOKEN")
    if publish_token:

...
impl ActorRepository {
    ...
    pub async fn apply_actor_to_engine(
        &self,
        engine: &mut talos_workflow_engine::ParallelWorkflowEngine,
        actor_id: Uuid,
    ) -> Result<()> {
        engine.set_actor_id(actor_id);
        let tier = match self.get_actor_max_llm_tier(actor_id).await {
            Ok(Some(t)) => t,
            Ok(None) => {
                ...
                engine.set_max_llm_tier(talos_workflow_engine_core::LlmTier::Tier1);
            ...
            Err(e) => {
                ...
                engine.set_max_llm_tier(talos_workflow_engine_core::LlmTier::Tier1);
                return Err(
                    e.context("apply_actor_to_engine: failed to resolve actor tier ceiling")
                );
            }
        };
    ...
    /// `Tier2` — see `apply_actor_to_engine` for the fail-closed contract.

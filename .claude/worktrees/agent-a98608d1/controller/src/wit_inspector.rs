    ...
    match config_schema {
        ...
        serde_json::Value::Object(map) => {
            let required: Vec<String> = map
                .iter()
                .filter_map(|(key, val)| {
                    // Mark as required if the schema says so or if no "default" key
                    val.as_object()
    ...
    match world {
        ...
        CapabilityWorld::Http => {
            "HTTP-capable node. Can make outbound HTTP requests, send webhooks, \
             execute GraphQL queries, send emails, render templates, and transform \
             data (CSV/XML). Cannot access raw TCP/UDP sockets, encrypted secrets, or the filesystem."
        }
        CapabilityWorld::Network => {
        ...
        CapabilityWorld::Secrets => {
            "Secrets-capable node. Same as network-node plus read-only access to the \
             encrypted secrets vault. Use for API key retrieval, token injection, and \

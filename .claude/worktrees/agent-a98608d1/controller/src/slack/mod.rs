...
mod rate_limiter;
pub use rate_limiter::SlackRateLimiter;
...
impl SlackApiClient {
    ...
    async fn resolve_user_mentions(&self, bot_token: &str, text: &str) -> String {
        ...
        let Ok(re) = regex::Regex::new(r"<@([A-Z0-9]+)>") else {
            return resolved;
        };
...
pub async fn list_channels_handler(
    Query(params): Query<SlackApiParams>,

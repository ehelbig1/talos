...
fn run(input: String) -> Result<String, String> {
    ...
    let token = match secrets::get_secret(secret_name) {
        Ok(t) => t,
        Err(_) => String::new()
    };

    logging::log(Level::Info, "4. Got secret (or empty)");
    ...
    let url = format!("https://api.github.com/repos/{}/commits?per_page=30", repo);
    let mut headers = vec![
    ...
    let commits: Vec<GithubCommit> = serde_json::from_slice(&response.body)
        .map_err(|e| format!("Failed to parse GitHub JSON response (err: {})", e))?;

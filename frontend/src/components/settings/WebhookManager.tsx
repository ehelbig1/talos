...
function CreateWebhookDialog({ onClose }: CreateDialogProps) {
  ...
  const [result, setResult] = useState<{ webhookUrl: string; verificationToken?: string | null } | null>(null);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl animate-in fade-in duration-500">
        <div className="bg-surface-3/90 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-10 w-full max-w-lg shadow-2xl relative overflow-hidden">
          ...
          <div className="flex items-center gap-5 mb-10 relative z-10">
            ...
            <div>
              <h3 className="text-xl font-black text-white tracking-tight font-outfit uppercase">Webhook Synthesized</h3>
              <p className="text-[10px] text-muted-foreground/40 font-bold uppercase tracking-widest mt-1">Persist these parameters securely</p>
            </div>
          </div>

          <div className="space-y-6 relative z-10">
            <div className="space-y-2">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/40 ml-1">Uplink URL</p>
              <div className="flex items-center gap-4 bg-black/40 border border-white/5 rounded-2xl px-5 py-4 shadow-inner group">
                <code className="text-xs font-mono text-primary flex-1 truncate selection:bg-primary/30">{result.webhookUrl}</code>
                <CopyButton value={result.webhookUrl} />
              </div>
            </div>
            {result.verificationToken && (
              <div className="space-y-2">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/40 ml-1">Verification Token</p>
                <div className="flex items-center gap-4 bg-black/40 border border-white/5 rounded-2xl px-5 py-4 shadow-inner group">
                  <code className="text-xs font-mono text-warning/80 flex-1 truncate selection:bg-warning/30">{result.verificationToken}</code>
      ...
      <div className="bg-surface-3/90 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-10 relative z-10">
          <div className="space-y-1">
            <h3 className="text-xl font-black text-white tracking-tight font-outfit uppercase">Uplink Config</h3>
            <p className="text-[10px] text-muted-foreground/40 font-bold uppercase tracking-[0.2em]">New Webhook Trigger</p>
          </div>
          <button type="button" onClick={onClose} aria-label="Close" className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl transition-premium text-muted-foreground hover:text-white">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="space-y-6 relative z-10">
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/40 ml-1">Protocol Alias</label>
            <input
              value={name}
          ...
          <div className="space-y-2">
            ...
            >
              <option value="" className="bg-surface-3">SELECT_DESTINATION...</option>
              {workflowsData?.map((w) => (
                <option key={w.id} value={w.id} className="bg-surface-3">{w.name.toUpperCase()}</option>
              ))}
            </select>
          </div>
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {createMutation.isPending ? "INITIALIZING..." : "SYNTHESIZE UPLINK"}
            </Button>
            <Button variant="ghost" className="px-6 h-14 rounded-2xl text-[10px] font-black tracking-widest" onClick={onClose}>
              ABORT
            </Button>
          </div>
        </div>
...
export default function WebhookManager() {
  ...
  return (
    <>
      {showCreate && <CreateWebhookDialog onClose={() => setShowCreate(false)} />}
      ...
      <div className="bg-surface-3/40 backdrop-blur-3xl border border-white/5 rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-12 relative z-10">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h3 className="text-2xl md:text-3xl font-black text-white tracking-tighter font-outfit uppercase leading-tight">Webhook Ingress</h3>
        ...
        {triggers.length === 0 ? (
          <div className="text-center py-24 bg-white/[0.01] border border-dashed border-white/5 rounded-[2.5rem] relative group overflow-hidden">
            ...
            <p className="text-sm text-muted-foreground font-black uppercase tracking-[0.2em]">No Active Uplinks</p>
            <p className="text-[10px] text-muted-foreground/30 font-bold uppercase tracking-widest mt-2 max-w-xs mx-auto leading-relaxed">
              Synthesize a webhook trigger to establish a secure ingress bridge to your logic modules.
            </p>
          </div>
        ) : (
          <div className="space-y-4 relative z-10">
            {triggers.map((t) => {
              const successRate = t.triggerCount > 0
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-6 mb-5 relative z-10">
                    <div className="flex items-center gap-4 min-w-0">
                      <div className={cn(
                        ...
                        t.enabled ? "bg-success animate-status-pulse" : "bg-muted-foreground/30",
                      )} />
                      <div className="min-w-0">
                        <span className="text-lg font-black text-white tracking-tight font-outfit uppercase truncate group-hover:text-primary transition-premium">{t.name}</span>
                    ...
                    <div className="flex items-center gap-6 shrink-0 pt-1">
                      {successRate !== null && (
                        <div className={cn(
                          ...
                          <span className="text-[9px] font-black uppercase tracking-widest opacity-40 mb-1">HEALTH_SCORE</span>
                      ...
                      {t.lastTriggeredAt && (
                        <div className="flex flex-col items-end text-muted-foreground">
                          <span className="text-[9px] font-black uppercase tracking-widest opacity-40 mb-1">LAST_SIGNAL</span>
                  ...
                  <div className="flex items-center gap-4 bg-black/40 border border-white/5 rounded-2xl px-5 py-4 shadow-inner relative z-10">
                    <code className="text-xs font-mono text-primary/80 flex-1 truncate selection:bg-primary/30 tracking-tight">{t.webhookUrl}</code>
        ...
        <div className="mt-10 pt-6 border-t border-white/5 flex items-center justify-between relative z-10">
          ...
          <span className="text-[10px] text-muted-foreground/20 font-black uppercase tracking-widest">
            {triggers.length} Active Protocol Bridge{triggers.length !== 1 ? "s" : ""}
          </span>
        </div>
      </div>
    </>

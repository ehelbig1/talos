...
export default function McpServerSettings() {
  ...
  const fetchAgents = async () => {
    try {
      ...
      if (import.meta.env.DEV) console.error("Failed to fetch MCP agents:", err);
  ...
  const { mutate: registerAgent, isPending: creating } =
    useRegisterMcpAgentMutation({
      ...
      onError: (err: Error) => {
        toast.error(sanitizeErrorMessage(err.message || "Failed to provision agent"));
      },
    });
  ...
  const inputBase = "w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-4 text-xs font-black uppercase tracking-widest text-white placeholder:text-muted-foreground/20 focus:outline-none focus:border-primary/40 focus:ring-4 focus:ring-primary/10 transition-premium shadow-inner";
  const selectBase = "w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-4 text-xs font-black uppercase tracking-widest text-white focus:outline-none focus:border-primary/40 focus:ring-4 focus:ring-primary/10 transition-premium shadow-inner appearance-none cursor-pointer";
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
          ...
          <div className="flex items-center gap-4">
             <div className="flex flex-col items-end gap-1">
                <span className="text-[9px] text-muted-foreground/30 font-black uppercase tracking-[0.2em]">SCHEMA_VERSION</span>
                <code className="text-[11px] font-mono text-primary font-bold">V1.2.0-STABLE</code>
             </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-surface-3/30 border border-white/5 rounded-[2.5rem] p-8 shadow-2xl backdrop-blur-xl relative overflow-hidden group">
            ...
            <div className="space-y-6 relative z-10">
              <div className="space-y-2.5">
                <label className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/30 ml-1">Protocol Identifier</label>
              ...
              <div className="space-y-2.5">
                <label className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/30 ml-1">Operational Role</label>
                <div className="relative group/input">
                    ...
                    {AVAILABLE_ROLES.map((r) => (
                        <option key={r.name} value={r.name} className="bg-surface-3">
                        {r.name.toUpperCase()}
                        </option>
                    ))}
                    </select>
            ...
            <div className="space-y-4">
               <div className="p-5 bg-black/40 border border-white/5 rounded-2xl space-y-3 shadow-inner group">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/30">Local_Dev</span>
                    <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded bg-primary/10 text-primary border border-primary/20">NO_AUTH</span>
                  </div>
                  <code className="text-[10px] font-mono text-primary/60 block truncate">{localEndpoint}</code>
                  <button onClick={() => { copy(localEndpoint); toast.success("Local endpoint copied"); }} className="w-full py-2 text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 hover:text-white hover:bg-white/5 rounded-lg border border-white/5 transition-premium">COPY_LOCAL_URI</button>
               </div>

               <div className="p-5 bg-black/40 border border-white/5 rounded-2xl space-y-3 shadow-inner group">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/30">Production</span>
                    <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded bg-warning/10 text-warning border border-warning/20">JWT_REQUIRED</span>
                  </div>
                  <code className="text-[10px] font-mono text-warning/60 block truncate">{authEndpoint}</code>
                  <button onClick={() => { copy(authEndpoint); toast.success("Production endpoint copied"); }} className="w-full py-2 text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 hover:text-white hover:bg-white/5 rounded-lg border border-white/5 transition-premium">COPY_AUTH_URI</button>
               </div>
            </div>
          </div>
        </div>
           ...
           <div className="bg-surface-3/30 border border-white/5 rounded-[3rem] p-10 shadow-2xl backdrop-blur-xl relative overflow-hidden min-h-[400px]">
              <div className="flex items-center justify-between mb-10">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h4 className="text-xl font-black text-white tracking-tight uppercase">Registry</h4>
                        <p className="text-[9px] text-muted-foreground/30 font-black uppercase tracking-[0.3em]">Active Protocol Entities</p>
                    </div>
                </div>
                {agents.length > 0 && (
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-2xl font-black text-white tracking-tighter leading-none">{agents.length}</span>
                    <span className="text-[9px] text-muted-foreground/30 font-black uppercase tracking-widest">ACTIVE_AGENTS</span>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {loading ? (
                  <div className="col-span-full py-24 flex flex-col items-center justify-center gap-4">
                    ...
                    <p className="text-[10px] text-muted-foreground/40 font-black uppercase tracking-[0.4em]">Synchronizing Registry...</p>
                  </div>
                ) : agents.length === 0 ? (
                  <div className="col-span-full py-24 flex flex-col items-center justify-center border border-dashed border-white/5 rounded-[2.5rem] bg-white/[0.01] group">
                    ...
                    <p className="text-[11px] text-muted-foreground/30 font-black uppercase tracking-[0.2em]">No Operational Agents Detected</p>
                  </div>
                ) : (
                  agents.map((agent) => (
                    <div
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between relative z-10 mb-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h5 className="text-sm font-black text-white tracking-tight uppercase group-hover:text-primary transition-premium">{agent.name}</h5>
                                <p className="text-[8px] font-mono text-muted-foreground/20 mt-1 uppercase">ID: {agent.id.slice(0, 16)}...</p>
                            </div>
                        </div>
                        <button
                        ...
                        >
                          {revokingId === agent.id ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />}
                        </button>
                      </div>

                      <div className="grid grid-cols-2 gap-4 relative z-10">
                        <div className="flex flex-col gap-1">
                            <span className="text-[8px] text-muted-foreground/20 font-black uppercase tracking-widest">INITIALIZED</span>
                        ...
                        <div className="flex flex-col gap-1">
                            <span className="text-[8px] text-muted-foreground/20 font-black uppercase tracking-widest">LAST_UPLINK</span>
                            <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground font-black uppercase tracking-tighter">
                                <Clock size={12} className="opacity-30" />
                                {agent.lastUsedAt ? new Date(agent.lastUsedAt).toLocaleDateString() : "NEVER"}
                            </div>
                        </div>
                      </div>
                    </div>
    ...
    "talos": {
      ...
      "env": {
        "TALOS_TOKEN": "YOUR_PROTOCOL_TOKEN"
                ...
                <div key={idx} className="bg-surface-3/30 border border-white/5 rounded-[2.5rem] p-8 shadow-2xl backdrop-blur-xl relative overflow-hidden group/guide">
                  <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-50" />
                  
                  <div className="flex items-center justify-between mb-6 relative z-10">
                    <div className="space-y-1">
                        <h5 className="text-[11px] font-black text-white uppercase tracking-[0.2em]">{inst.title}</h5>
                        <p className="text-[9px] text-muted-foreground/30 font-black uppercase tracking-widest">{inst.desc}</p>
                    </div>
                    <button
                      onClick={() => { copy(inst.config); toast.success("Configuration copied"); }}
                  ...
                  <div className="relative z-10">
                    <div className="absolute top-4 right-6 text-[8px] font-black text-primary/40 uppercase tracking-[0.3em] pointer-events-none">JSON_SPEC</div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
           <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 flex-1">
              {[
                ...
                <div key={tool.name} className="flex items-center gap-4 group/tool">
                    ...
                    <span className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] group-hover/tool:text-white transition-premium">{tool.name}</span>
                </div>
              ))}
           </div>
           ...
           <div className="max-w-md">
              <div className="flex items-center gap-3 mb-2 text-primary">
                <Info size={16} />
                <span className="text-[10px] font-black uppercase tracking-[0.2em]">PROTOCOL_ADVISORY</span>
              </div>
              <p className="text-[10px] text-muted-foreground/40 font-medium leading-relaxed uppercase tracking-widest">
                MCP agents operate with delegated authority. Use short-lived tokens and granular roles to maintain operational security perimeters.
              </p>
           </div>
        </div>
      </div>

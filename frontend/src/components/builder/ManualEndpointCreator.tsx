    ...
    return (
      <Button
        ...
        className={cn(
          ...
          baseUrl ? "bg-emerald-500 hover:bg-emerald-400 text-white shadow-lg shadow-emerald-500/20" : "bg-surface-3 text-muted-foreground/40 grayscale"
        )}
      >
      ...
      <div className="flex items-center justify-between relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
              ...
              <p className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-widest">Manual Protocol Definition</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="space-y-6 relative z-10">
        ...
        <div className="flex gap-4">
          <div className="w-32 space-y-2">
            <label className="text-[9px] font-black text-white/40 uppercase tracking-widest px-1">Method</label>
            <select
          ...
          <div className="flex-1 space-y-2">
            <label className="text-[9px] font-black text-white/40 uppercase tracking-widest px-1">Path <span className="text-emerald-400">*</span></label>
            <input
              type="text"
        ...
        <div className="space-y-2">
          <label className="text-[9px] font-black text-white/40 uppercase tracking-widest px-1">Operational Description</label>
          <input
            type="text"
            value={currentEndpoint.summary}
            onChange={(e) => setCurrentEndpoint({ ...currentEndpoint, summary: e.target.value })}
        ...
        <div className="space-y-4 pt-4 border-t border-white/5">
          <div className="flex justify-between items-center px-1">
            <h5 className="text-[10px] font-black text-white/60 uppercase tracking-[0.2em]">Operational Parameters</h5>
          ...
          <div className="space-y-3">
            {parameters.length === 0 ? (
              <div className="py-10 text-center bg-surface-4 border border-white/5 rounded-[2rem]">
                  <p className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-[0.3em]">Null Parameters Defined</p>
              </div>
            ) : (
              <div className="space-y-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                {parameters.map((param, index) => (
                  <div key={index} className="flex gap-3 p-4 bg-surface-3/60 border border-white/5 rounded-2xl group/param items-center transition-premium hover:bg-surface-4">
                    <Badge className={cn(
                        "w-16 h-8 text-[7px] font-black uppercase tracking-widest border",
                        param.in === "path" ? "bg-blue-500/10 text-blue-400 border-blue-500/20" : 
                        param.in === "query" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : 
                    ...
                    <label className="flex items-center gap-2 cursor-pointer group/check">
                      ...
                      />
                      <span className="text-[8px] font-black text-muted-foreground/20 uppercase tracking-widest group-hover/check:text-white transition-premium">Req</span>
                    </label>
                    <button
                      type="button"
        ...
        <div className="flex gap-4 pt-6">
          <Button
            ...
            className={cn(
              ...
              currentEndpoint.path ? "bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-500/20" : "bg-surface-3 text-muted-foreground/40"
            )}
          >
        ...
        <div className="p-6 bg-emerald-500/5 border border-emerald-500/10 rounded-[2rem] flex items-start gap-4">
            ...
            <p className="text-[10px] text-emerald-400/60 font-bold uppercase tracking-widest leading-relaxed">
              Strategic Insight: Manual vector synthesis requires precise path variable identification. Use {`{variable_name}`} syntax for dynamic path injection.
            </p>
        </div>
      </div>
    </div>

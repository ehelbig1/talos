...
export function GoogleCalendarSelector({
  ...
  const fetchCalendars = async (integrationId: string) => {
    try {
      ...
      setError(sanitizeErrorMessage(err instanceof Error ? err.message : "Failed to load calendars"));
    } finally {
      ...
      <div className="flex items-center justify-between relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
              ...
              <p className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-widest">Temporal Synchronization Vector</p>
          </div>
        </div>
        {setupComplete && (
      ...
      {integrations.length === 0 ? (
        <div className="p-10 bg-surface-4 border-2 border-dashed border-white/5 rounded-[2rem] text-center flex flex-col items-center animate-in fade-in duration-700">
          ...
          <p className="mb-8 text-[11px] font-bold text-muted-foreground/40 uppercase tracking-widest leading-relaxed max-w-xs mx-auto">
            Establish a persistent auth tunnel in <strong>Command Settings</strong> to activate temporal monitoring.
          </p>
          <a 
            href="/settings" 
            target="_blank" 
                    ...
                    >
                      <div className="flex items-center gap-4 relative z-10">
                        <div className={cn(
                            ...
                            isSelected ? "bg-primary text-white border-primary" : "bg-white/5 text-muted-foreground/40 border-white/10"
                        )}>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <span className="block text-[9px] font-black text-muted-foreground/20 uppercase tracking-tighter mt-0.5">
                              Established: {new Date(integration.created_at).toLocaleDateString()}
                            </span>
                        </div>
                        {isSelected && (
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
              ...
              {loadingCalendars ? (
                <div className="p-10 flex flex-col items-center justify-center gap-4 bg-surface-3 border border-white/5 rounded-2xl">
                  ...
                  <p className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-widest">Deciphering Calendars...</p>
                </div>
              ) : calendars.length === 0 ? (
                <div className="p-6 bg-surface-3 border border-white/5 rounded-2xl text-center">
                  <p className="text-[10px] font-black text-destructive uppercase tracking-widest">Null Calendars Returned</p>
                </div>
              ) : (
                      ...
                      >
                        <div className="flex items-start gap-4">
                          <div className={cn(
                              ...
                              isSelected ? "bg-primary border-primary text-white" : "bg-white/5 border-white/10 group-hover:border-white/30"
                          )}>
                            {isSelected && <Check className="w-3 h-3" />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-1">
                                <span className={cn("text-[11px] font-black uppercase tracking-widest truncate transition-premium", isSelected ? "text-white" : "text-white/40")}>
                                  {calendar.summary}
                                </span>
                                {calendar.primary && (
              ...
              {selectedCalendars.length > 0 && !setupComplete && (
                ...
                >
                  Confirm Temporal Locking ({selectedCalendars.length} VECTOR{selectedCalendars.length > 1 ? "S" : ""})
                </Button>
              )}
              ...
              {setupComplete && (
                <div className="p-6 bg-emerald-500/5 border border-emerald-500/20 rounded-[2rem] mt-6 animate-in zoom-in-95 duration-500">
                  ...
                  <p className="m-0 text-[11px] font-bold text-muted-foreground/40 leading-relaxed uppercase tracking-widest">
                    Selected {selectedCalendars.length} monitor cycle{selectedCalendars.length > 1 ? "s" : ""}. 
                    Temporal watch channels will initialize upon node synthesis. 
                    Real-time telemetry will stream to the workflow core.
                  </p>
                </div>
              )}
            </div>

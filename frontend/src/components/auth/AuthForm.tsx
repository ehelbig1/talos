...
export function AuthForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(sanitizeErrorMessage(err instanceof Error ? err.message : "Login failed"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleTwoFactorSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(sanitizeErrorMessage(err instanceof Error ? err.message : "Invalid verification code"));
    } finally {
      setIsLoading(false);
    }
  };

  const labelClass = "block mb-2 text-[10px] font-black text-muted-foreground/40 uppercase tracking-[0.2em] ml-1";
        ...
        <div className="bg-surface-1/60 border border-white/5 rounded-[2.5rem] p-10 shadow-2xl glass-dark backdrop-blur-3xl relative overflow-hidden">
          ...
          <div className="text-center mb-10 relative">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-[2rem] bg-primary/10 border border-primary/20 mb-6 shadow-[0_0_30px_hsla(var(--primary),0.2)] group hover:scale-110 transition-premium">
              <Zap className="w-8 h-8 text-primary animate-status-pulse" fill="currentColor" />
            </div>
            ...
            >
              ...
              <div>
                ...
                <p className="mt-3 text-[9px] text-muted-foreground/30 font-bold uppercase tracking-widest leading-relaxed px-1">
                  Requires 12+ characters, mixed case, numeric & tactical symbols.
                </p>
              </div>
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-[10px] text-muted-foreground/40 font-black uppercase tracking-widest leading-relaxed">
                  ...
                  <br />Enter 6-digit terminal code.
                </p>
              </div>
          ...
          <div className="flex items-center gap-6 my-10 opacity-20">
            ...
            <span className="text-[9px] text-white font-black tracking-[0.3em] uppercase">Uplink</span>
          ...
          {step === "credentials" && (
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <svg width="18" height="18" viewBox="0 0 20 20" className="grayscale group-hover:grayscale-0 transition-premium">
                  <path d="M19.6 10.23c0-.82-.1-1.42-.25-2.05H10v3.72h5.5c-.15.96-.74 2.31-2.04 3.22v2.45h3.16c1.89-1.73 2.98-4.3 2.98-7.34z" fill="#4285F4" />
                  <path d="M13.46 15.13c-.83.59-1.96 1-3.46 1-2.64 0-4.88-1.74-5.68-4.15H1.07v2.52C2.72 17.75 6.09 20 10 20c2.7 0 4.96-.89 6.62-2.42l-3.16-2.45z" fill="#34A853" />
                  <path d="M3.99 10c0-.69.12-1.35.32-1.97V5.51H1.07A10.02 10.02 0 000 10c0 1.61.39 3.14 1.07 4.49l3.24-2.52c-.2-.62-.32-1.28-.32-1.97z" fill="#FBBC05" />
              ...
              >
                <svg width="18" height="18" viewBox="0 0 20 20" className="grayscale group-hover:grayscale-0 transition-premium">
                  ...
                  <path d="M10 4c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6 2.69-6 6-6z" fill="white" />
                  <path d="M10 7a3 3 0 100 6 3 3 0 000-6z" fill="#007DC1" />
                </svg>
                <span className="text-[10px] font-black uppercase tracking-widest">Okta</span>
              </button>
            </div>
          )}
          ...
          {mode === "signup" && (
            <p className="mt-8 text-center text-[9px] text-muted-foreground/30 font-bold uppercase tracking-[0.1em] leading-relaxed">
              BY ENROLLING, YOU ADHERE TO THE TALOS<br />
              <span className="text-muted-foreground/50 hover:text-white transition-premium cursor-pointer">OPERATIONAL DIRECTIVE</span> & <span className="text-muted-foreground/50 hover:text-white transition-premium cursor-pointer">PRIVACY PROTOCOL</span>.
            </p>
          )}
        </div>

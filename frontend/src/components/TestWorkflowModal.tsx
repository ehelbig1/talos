...
import { useTestWorkflowMutation, TestWorkflowMutation } from "@/generated/graphql";
...
function NodeTraceCard({ trace, index, nodeLabel }: { trace: NodeTrace; index: number; nodeLabel?: string }) {
  const [expanded, setExpanded] = useState(trace.status === "failed");

  const statusConfig = {
    completed: { icon: <CheckCircle2 className="w-3 h-3" />, color: "text-success bg-success/10 border-success/20" },
    failed: { icon: <XCircle className="w-3 h-3" />, color: "text-destructive bg-destructive/10 border-destructive/20" },
    skipped: { icon: <SkipForward className="w-3 h-3" />, color: "text-muted-foreground bg-white/5 border-white/10" },
  }[trace.status] ?? { icon: <Zap className="w-3 h-3" />, color: "text-muted-foreground bg-white/5 border-white/10" };

  return (
    <div className={cn(
      "rounded-2xl border overflow-hidden transition-premium",
      trace.status === "failed" ? "border-destructive/30 bg-destructive/5" : "border-white/5 bg-white/[0.02]",
    )}>
      <button
      ...
      >
        ...
        <div className={cn("flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[9px] font-black border shrink-0 uppercase tracking-widest", statusConfig.color)}>
          {statusConfig.icon}
          {trace.status}
        </div>
        <span className="text-xs font-bold text-foreground/80 flex-1 truncate uppercase tracking-tight">
          {nodeLabel ?? <code className="font-mono text-muted-foreground/40">{trace.nodeId.slice(0, 8)}</code>}
        </span>
        ...
        <div className="border-t border-white/5 px-6 pb-6 space-y-5 pt-5 animate-in slide-in-from-top-2 duration-300">
          {trace.error && (
            <div className="bg-destructive/10 border border-destructive/20 rounded-[1.25rem] px-5 py-4 shadow-inner">
              <p className="text-[9px] font-black text-destructive uppercase tracking-[0.2em] mb-2">Diagnostic Fault</p>
              <p className="text-[11px] text-destructive/80 font-mono leading-relaxed font-bold">{sanitizeErrorMessage(trace.error)}</p>
            </div>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <p className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] ml-1">Ingress Context</p>
              ...
              <div className="space-y-3">
                <p className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] ml-1">Egress result</p>
                {trace.output ? (
                    ...
                    <div className="h-full flex items-center justify-center border border-white/5 bg-white/[0.02] rounded-[1.25rem] opacity-20">
                        <p className="text-[10px] font-black uppercase tracking-widest italic">No Output Protocol</p>
                    </div>
                )}
              </div>
          </div>
...
export function TestWorkflowModal({ workflowId, workflowName, onClose }: Props) {
  ...
  const handleRun = () => {
    if (jsonError) return;
    setResult(null);
    testMutation.mutate({ workflowId, mockInputs: mockInputs !== "{}" ? mockInputs : undefined });
  };
      ...
      <div className="space-y-8 relative z-10 p-2 -mt-4">
        <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-black text-emerald-500/60 uppercase tracking-[0.2em]">Sandboxed Protocol</span>
            </div>
        </div>
        ...
        <div className="bg-surface-4/40 border border-white/5 rounded-[2rem] p-8 space-y-6 shadow-inner glass-light">
          <div className="flex items-center justify-between">
            ...
            {jsonError && <span className="text-[9px] font-black text-destructive uppercase tracking-widest animate-pulse">{jsonError}</span>}
          </div>
          <textarea
            value={mockInputs}
          ...
          <div className="flex items-center justify-between gap-6">
            <div className="flex items-start gap-3 flex-1">
                ...
                Executes the current DAG in a transient sandbox. Zero persistence, zero external side-effects.
                </p>
            </div>
            <Button
              onClick={handleRun}
        ...
        <div className="max-h-[460px] overflow-y-auto custom-scrollbar border border-white/5 rounded-[2rem] bg-surface-4/20 shadow-inner">
          {!result && !testMutation.isPending && (
            <div className="flex flex-col items-center justify-center py-24 text-center px-6 opacity-20">
              ...
              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em]">Awaiting Diagnostic Initiation</p>
            </div>
          )}
          ...
          {testMutation.isPending && (
            <div className="flex flex-col items-center justify-center py-24 gap-6">
              ...
              <p className="text-[10px] font-black text-primary/40 uppercase tracking-[0.4em] animate-pulse">Processing Execution Trace...</p>
            </div>
          )}
            ...
            <div className="p-8 space-y-10">
                ...
                <div className="flex items-center gap-4 ml-auto">
                  <div className="flex flex-col items-end gap-1">
                      <div className="flex items-center gap-3">
                          {succeeded > 0 && <span className="text-success text-[10px] font-black uppercase tracking-widest">{succeeded} SUCCESS</span>}
                          {failed > 0 && <span className="text-destructive text-[10px] font-black uppercase tracking-widest">{failed} FAILURE</span>}
                          {skipped > 0 && <span className="text-muted-foreground/40 text-[10px] font-black uppercase tracking-widest">{skipped} SKIPPED</span>}
                      </div>
                      <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden flex shadow-inner">
                          <div className="bg-success h-full transition-premium" style={{ width: `${(succeeded / result.nodeTraces.length) * 100}%` }} />
                          <div className="bg-destructive h-full transition-premium" style={{ width: `${(failed / result.nodeTraces.length) * 100}%` }} />
                      </div>
                  </div>
                </div>
              </div>
              ...
              {result.error && (
                <div className="bg-destructive/5 border border-destructive/20 rounded-[2rem] p-6 flex items-start gap-4 shadow-2xl glass-light">
                  ...
                  <div className="space-y-2">
                      <p className="text-[10px] font-black text-destructive uppercase tracking-[0.2em]">Orchestration Failure</p>
                      <p className="text-[11px] text-destructive/80 leading-relaxed font-mono font-bold">{sanitizeErrorMessage(result.error)}</p>
                  </div>
                </div>
              )}
              ...
              {result.schemaWarnings.length > 0 && (
                <div className="bg-warning/5 border border-warning/15 rounded-[2rem] p-6 space-y-4 shadow-2xl glass-light">
                  <p className="text-[10px] font-black text-warning uppercase tracking-[0.2em] ml-1">Protocol Schema Warnings</p>
                  <div className="space-y-2">
                      {result.schemaWarnings.map((w) => (
                        <p key={w} className="text-[11px] text-warning/60 leading-relaxed font-bold">• {w}</p>
                      ))}
                  </div>
                </div>
              )}

...
const STATUS_DOT: Record<NodeStatusType, string> = {
  ...
  running: "bg-primary animate-status-pulse shadow-[0_0_10px_hsla(var(--primary),0.5)]",
  success: "bg-success shadow-[0_0_10px_hsla(var(--success),0.5)]",
  failed: "bg-destructive shadow-[0_0_10px_hsla(var(--destructive),0.5)]",
  awaiting_approval: "bg-warning animate-pulse shadow-[0_0_10px_hsla(var(--warning),0.5)]",
};
...
export const NodeInspector: React.FC<NodeInspectorProps> = ({
  ...
  const copyNodeIdTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const copyModuleIdTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    return () => {
      if (copyNodeIdTimeoutRef.current) clearTimeout(copyNodeIdTimeoutRef.current);
      if (copyModuleIdTimeoutRef.current) clearTimeout(copyModuleIdTimeoutRef.current);
    };
  }, []);
  ...
  const handleCopyNodeId = async () => {
    try {
      ...
      if (copyNodeIdTimeoutRef.current) clearTimeout(copyNodeIdTimeoutRef.current);
      copyNodeIdTimeoutRef.current = setTimeout(() => setCopiedNodeId(false), 2000);
  ...
  const handleCopyModuleId = async () => {
    if (node.data.moduleId) {
      try {
        ...
        if (copyModuleIdTimeoutRef.current) clearTimeout(copyModuleIdTimeoutRef.current);
        copyModuleIdTimeoutRef.current = setTimeout(() => setCopiedModuleId(false), 2000);
  ...
  return (
    <div className="flex flex-col h-full bg-surface-2/80 backdrop-blur-3xl relative border-l border-white/5 shadow-[-20px_0_50px_rgba(0,0,0,0.4)] animate-in slide-in-from-right duration-500">
      ...
      >
                        ...
                        <div className="space-y-6">
                            <FormField label="Conditional Skip (Rhai Script)">
                                <Textarea
                                    ...
                                    onChange={(e) =>
                                    updateNodeData(node.id, { skipCondition: e.target.value })
                                    }
                                />
                            </FormField>
                            ...
                            <div className="flex items-center justify-between p-6 rounded-2xl bg-black/40 border border-white/5 shadow-xl">
                                ...
                                <div 
                                    onClick={() => updateNodeData(node.id, { continueOnError: !node.data.continueOnError })}
                                    className={cn(
                                        "w-12 h-6 rounded-full border p-1 cursor-pointer transition-premium relative",
                                        node.data.continueOnError ? "bg-primary/20 border-primary/40" : "bg-white/5 border-white/10"
                                    )}
                                >
                                    <div className={cn(
                                        "w-4 h-4 rounded-full transition-premium shadow-lg",
                                        node.data.continueOnError ? "bg-primary translate-x-6" : "bg-white/20 translate-x-0"
                                    )} />
                                </div>
                            </div>
        ...
        <TabsContent value="logs" className="flex-1 overflow-auto p-8 space-y-10 custom-scrollbar focus:outline-none">
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="p-6 rounded-[2rem] bg-surface-3/40 border border-white/5 shadow-2xl glass-dark relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between relative z-10">
                <div className="flex items-center gap-5">
                    <div className={cn("w-3 h-3 rounded-full relative", STATUS_DOT[status])}>
                        <div className={cn("absolute inset-0 rounded-full animate-ping opacity-20", STATUS_DOT[status])} />
                    </div>
                ...
                <div className="flex flex-col items-end gap-1">
                    ...
                    <div className="flex gap-1">
                        {[...Array(5)].map((_, i) => (
                            <div key={i} className="w-1 h-3 bg-primary/20 rounded-full animate-pulse" style={{ animationDelay: `${i * 150}ms` }} />
                        ))}
                    </div>
                </div>
              </div>
              ...
              <div className="p-8 rounded-[2rem] bg-destructive/5 border border-destructive/10 space-y-5 shadow-2xl relative overflow-hidden group">
                ...
                {fixSuggestion && (
                  <div className="p-5 bg-warning/5 border border-warning/10 rounded-2xl text-warning relative z-10 flex gap-4 shadow-xl">
                    ...
                    <div className="space-y-1">
                        <span className="text-[9px] font-black uppercase tracking-widest opacity-40 block">HEAL_DIRECTIVE</span>
                        <p className="text-[11px] font-black uppercase tracking-tight leading-relaxed italic">{fixSuggestion}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        await navigator.clipboard.writeText(JSON.stringify(nodeResult, null, 2));
                    ...
                    >
                      ...
                      <div className="flex-1 space-y-2">
                        <div className="flex items-center gap-3">
                            {ev.status === "SUCCESS" && <CheckCircle2 className="w-3.5 h-3.5 text-success/60" />}
                            <span
                            className={cn(

...
function StatusPill({ status }: { status: string }) {
  const configs: Record<string, string> = {
    completed: "bg-success/5 text-success border-success/10 shadow-[0_0_10px_hsla(var(--success),0.1)]",
    failed: "bg-destructive/5 text-destructive border-destructive/10 shadow-[0_0_10px_hsla(var(--destructive),0.1)]",
    running: "bg-primary/5 text-primary border-primary/10 shadow-[0_0_10px_hsla(var(--primary),0.1)]",
    awaiting_approval: "bg-warning/5 text-warning border-warning/10 shadow-[0_0_10px_hsla(var(--warning),0.1)]",
    pending: "bg-warning/5 text-warning border-warning/10",
    timeout: "bg-warning/5 text-warning border-warning/10",
  };

  const cls = configs[status] ?? "bg-surface-4 text-muted-foreground/20 border-white/5";
  ...
  ({
    ...
    return (
      <div className={cn(
          ...
          isExpanded ? "border-primary/20 ring-1 ring-primary/10 bg-surface-2/60" : "hover:bg-surface-2/40 hover:border-white/10"
      )}>
        {/* Row header */}
        <button
              ...
              <div className="relative group/error">
                ...
                <div className="relative p-5 bg-destructive/5 rounded-2xl border border-destructive/20 text-[11px] font-bold text-destructive shadow-inner leading-relaxed animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="font-black uppercase tracking-[0.3em] text-[9px]">CRITICAL_FAULT_TRACE</span>
                    </div>
              ...
              >
                {loadingLogs ? (
                  ...
                  <div className="py-8 flex flex-col items-center justify-center opacity-20 grayscale">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest">No Events Captured</p>
                  </div>
                ) : (
...
export const ExecutionHistory = React.memo(function ExecutionHistory({
  ...
  const {
    ...
    },
    {
      enabled: !!moduleId,
      staleTime: 0,
      refetchInterval: (query: { state: { data?: GetModuleExecutionHistoryQuery } }) => {
    ...
    <div className="mt-12 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="flex items-center justify-between px-2">
        <div className="flex items-center gap-5">
            <div className={cn(
                ...
                hasRunning && "border-primary/40 shadow-[0_0_20px_hsla(var(--primary),0.2)] animate-status-pulse"
            )}>
                {hasRunning ? (
            ...
            <div>
                ...
                <div className="flex items-center gap-3">
                    <span className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-[0.3em]">
                        {hasRunning ? "Synchronizing Telemetry..." : "Historical Trace Log"}
                    </span>
                    {hasRunning && (
        ...
        >
          <RefreshCw className={cn("w-4 h-4 mr-2.5", isFetching && "animate-spin")} />
          Rescan Archive
        </Button>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-32 gap-6 opacity-20 grayscale">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em]">Decoding Sequence Trace...</p>
        </div>
      ) : history.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-32 bg-surface-2/20 border border-white/5 border-dashed rounded-[3rem] animate-in zoom-in-95 duration-1000">
           ...
           <p className="text-[10px] font-black text-muted-foreground/20 uppercase tracking-[0.4em]">Zero Execution Cycles Logged</p>
        </div>
      ) : (

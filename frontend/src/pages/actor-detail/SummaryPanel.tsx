...
import {
  Sparkles, Pencil, Play, Pause, Square, Activity, Save,
  CalendarClock, Copy,
} from "lucide-react";
import { cn } from "@/lib/utils";
import { sanitizeErrorMessage } from "@/lib/sanitize";
import { ConfirmDialog } from "@/components/ui";
import { updateActor, cloneActor, type ActorDetails, type ActorActionLogEntry, type ActorWorkflowItem } from "@/lib/graphqlClient";
import { getCapabilityConfig, isAiWorkflow, CAPABILITY_WORLDS } from "@/lib/capabilityConfig";
import { ageDays } from "@/lib/formatTime";
import {
  statusColors, CapabilityBadge, StatCard, ACTION_ICONS,
  humanizeLogEntry, relativeTime,
} from "./shared";
...
export function SummaryPanel({
  ...
  const { mutate: doUpdate, isPending: updatePending } = useMutation({
    mutationFn: () =>
      updateActor(actor.id.toString(), {
        ...
        maxCapabilityWorld: editWorld !== actor.maxCapabilityWorld ? editWorld : undefined,
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["actor", actor.id.toString()] });
      setEditing(false);
  ...
  const { mutate: doClone, isPending: clonePending } = useMutation({
    ...
    onSuccess: (cloned) => {
      ...
      toast.success(`Cloned as "${cloned.name}" — click to open`, {
        action: { label: "Open", onClick: () => navigate(`/actors/${cloned.id}`) },
        duration: 6000,
      });
    },
    onError: (e) => toast.error(sanitizeErrorMessage(String(e))),
  ...
  const ageLabel = ageDaysNum === 0 ? "Today" : ageDaysNum === 1 ? "1 day" : `${ageDaysNum} days`;
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <ConfirmDialog
        ...
        onConfirm={() => { setShowTerminateConfirm(false); onTerminate(); }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Identity Name</label>
                      <input
                        value={editName}
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Capability Ceiling</label>
                      <select
                        value={editWorld}
                      ...
                      >
                        {Object.entries(CAPABILITY_WORLDS).map(([key, cfg]) => (
                          <option key={key} value={key} className="bg-surface-4 text-white">
                            {cfg.label} Level
                          </option>
                        ))}
                      </select>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Functional Description</label>
                    <textarea
                      value={editDesc}
                ...
                <>
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {isAiActor && (
                          <span className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 text-primary text-[9px] font-black uppercase tracking-widest px-3 py-1 shadow-sm">
                            <Sparkles className="w-3 h-3" />AI Synthetic
                          </span>
                        )}
                      </div>
                      <h2 className="text-4xl font-black text-white tracking-tighter leading-none group-hover:text-primary transition-colors duration-500">{actor.name}</h2>
                      {actor.description ? (
                        <p className="text-muted-foreground font-medium text-lg leading-relaxed max-w-2xl">{actor.description}</p>
                      ) : (
                        <p className="text-muted-foreground/30 font-black uppercase tracking-widest italic text-xs">No active operational parameters defined.</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-8 pt-8 border-t border-white/5 flex flex-wrap gap-8">
                    <div className="space-y-1">
                      <div className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest">Protocol Created</div>
                    ...
                    {actor.lastActiveAt && (
                      <div className="space-y-1">
                        <div className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest">Last Activity</div>
                        <div className="text-sm font-black text-white flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <StatCard label="Cycles" value={actor.executionCount} accent="text-primary" />
            <StatCard label="Logic Nodes" value={actor.workflowCount} accent="text-sky-400" />
            <StatCard
              label="Auth Level"
            ...
            />
            <StatCard label="Persistence" value={ageLabel} accent="text-warning" />
          </div>
        </div>
          ...
          <div className="bg-surface-3/40 border border-white/5 rounded-[3rem] p-8 glass h-full">
            ...
            {recentEntries.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest">No activity detected.</p>
              </div>
            ) : (
              <div className="space-y-6">
                {recentEntries.map((entry) => (
                  <div key={entry.id} className="flex items-start gap-4 group/entry">
                    <div className="w-8 h-8 rounded-xl bg-surface-4/60 border border-white/5 flex items-center justify-center text-primary shrink-0 transition-premium group-hover/entry:bg-primary/10 group-hover/entry:border-primary/20">
                      {ACTION_ICONS[entry.actionType.toLowerCase()] ?? <Activity className="w-3.5 h-3.5" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-black text-white leading-snug truncate group-hover/entry:text-primary transition-colors">{humanizeLogEntry(entry)}</p>
                      <time className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-widest mt-1 block">{relativeTime(entry.timestamp)}</time>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {!isTerminated && (
              <div className="mt-12 pt-8 border-t border-white/5 space-y-4">
                <h3 className="text-xs font-black text-white uppercase tracking-widest mb-4">Identity Controls</h3>
                <button
                  onClick={onToggle}
                ...
                >
                  <span className="text-[10px] font-black uppercase tracking-widest">
                    {actor.status === "active" ? "Suspend Identity" : "Deploy Identity"}
                  </span>
                  {actor.status === "active" ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                </button>
              </div>
            )}
          </div>

...
import {
  Brain, Plus, Download, Upload, Filter, X, Save, Pencil, Trash2,
  ChevronDown, ChevronUp, Loader2,
} from "lucide-react";
import { cn } from "@/lib/utils";
import { sanitizeErrorMessage } from "@/lib/sanitize";
import {
  listActorMemories, writeActorMemory, deleteActorMemory,
...
const MEMORY_TYPE_META: Record<MemoryType, { label: string; ttl: string; color: string; bg: string }> = {
  semantic:   { label: "Semantic",   ttl: "Permanent", color: "text-violet-400",  bg: "bg-violet-500/10"  },
  episodic:   { label: "Episodic",   ttl: "7 days",    color: "text-sky-400",     bg: "bg-sky-500/10"     },
  working:    { label: "Working",    ttl: "1 hour",    color: "text-amber-400",   bg: "bg-amber-500/10"   },
  scratchpad: { label: "Scratchpad", ttl: "24 hours",  color: "text-emerald-400", bg: "bg-emerald-500/10" },
};

// ── TTL status ────────────────────────────────────────────────────────────────

function ttlStatus(expiresAt: string | null): { label: string; urgency: 0 | 1 | 2 | 3 } {
...
function MemoryTypeBadge({ type }: { type: string }) {
  const meta = MEMORY_TYPE_META[type as MemoryType] ?? {
    label: type, ttl: "", color: "text-muted-foreground", bg: "bg-surface-4/60",
  };
  return (
    <span className={cn(
...
function MemoryEntryRow({
  actorId, entry, onEdit, onDelete, deleting,
}: {
  actorId: string;
  ...
  try { prettyValue = JSON.stringify(JSON.parse(entry.value), null, 2); }
  catch { /* raw string */ }
  const isMultiline = prettyValue.includes("\n");

  const { label: ttlLabel, urgency } = ttlStatus(entry.expiresAt);
  const ttlColor =
    urgency === 3 ? "text-red-400 bg-red-500/10" :
    urgency === 2 ? "text-amber-400 bg-amber-500/10" :
    "text-muted-foreground/40 bg-surface-3/60";

  const handleExtend = async () => {
    setExtending(true);
    try {
      await writeActorMemory({ actorId, key: entry.key, value: entry.value, memoryType: entry.memoryType });
      queryClient.invalidateQueries({ queryKey: ["actorMemories", actorId] });
      toast.success(`Memory '${entry.key}' TTL refreshed`);
    } catch { toast.error("Failed to extend memory TTL"); }
    ...
    <div className="bg-background border border-white/5 rounded-xl px-4 py-3 space-y-1.5">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          {ttlLabel && (
            <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded-full", ttlColor)}>
              {ttlLabel}
            </span>
          )}
        </div>
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button onClick={() => onEdit(entry)} className="p-1 text-muted-foreground/40 hover:text-violet-400 transition-premium" aria-label="Edit">
            <Pencil className="w-3.5 h-3.5" />
          </button>
          <button onClick={() => onDelete(entry.key)} disabled={deleting} className="p-1 text-muted-foreground/40 hover:text-red-400 transition-premium disabled:opacity-40" aria-label="Delete">
...
function MemoryForm({
  actorId, initial, onSaved, onCancel,
}: {
  actorId: string;
  ...
  const [value, setValue] = useState(() => {
    if (!initial) return "";
    try { return JSON.stringify(JSON.parse(initial.value), null, 2); }
    catch { return initial.value; }
  });
  const [memType, setMemType] = useState<MemoryType>((initial?.memoryType as MemoryType) ?? "semantic");
  const [jsonError, setJsonError] = useState<string | null>(null);

  const { mutate, isPending } = useMutation({
    mutationFn: () => writeActorMemory({ actorId, key, value, memoryType: memType }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["actorMemories", actorId] });
    ...
    <div className="bg-surface-3/60 border border-violet-500/20 rounded-2xl px-6 py-5 space-y-4">
      ...
      <div className="space-y-1">
        <label className="text-xs text-muted-foreground font-medium">
          Type
          <span className="ml-2 text-muted-foreground/40 font-normal">({MEMORY_TYPE_META[memType].ttl})</span>
        </label>
        ...
        {memType === "semantic" && (
          <p className="text-[10px] text-violet-400/70 mt-1">
            ...
            <code className="font-mono">__actor_context__</code> when this actor triggers a workflow
            with inject_memory_context enabled.
          </p>
        )}
      </div>
      ...
      <div className="space-y-1">
        <label className="text-xs text-muted-foreground font-medium">
          Value <span className="text-muted-foreground/40 font-normal">(JSON)</span>
        </label>
        <textarea
          value={value}
      ...
      <div className="flex justify-end gap-2">
        <button onClick={onCancel} className="text-xs text-muted-foreground hover:text-white px-3 py-1.5 rounded-lg border border-white/10 hover:border-white/20 transition-premium">
          Cancel
        </button>
        <button
          onClick={() => mutate()}
          disabled={isPending || !key.trim() || !value.trim() || jsonError !== null}
...
export function MemoryPanel({ actorId }: { actorId: string }) {
  ...
  const filtered = useMemo(() => {
    let result = filterType === "all" ? memories : memories.filter((m) => m.memoryType === filterType);
    const q = memSearch.trim().toLowerCase();
    if (q) result = result.filter((m) => m.key.toLowerCase().includes(q) || m.value.toLowerCase().includes(q));
    return result;
  ...
  const handleExport = () => {
    const payload = memories.map((m) => ({ key: m.key, value: m.value, memoryType: m.memoryType }));
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!Array.isArray(parsed)) { toast.error("Expected a JSON array of memory entries"); return; }

    const entries = parsed as Array<{ key?: unknown; value?: unknown; memoryType?: unknown }>;
    const valid = entries.filter((e) => typeof e.key === "string" && e.key.trim() && typeof e.value === "string");
    if (valid.length === 0) { toast.error("No valid entries found (each needs key and value strings)"); return; }
    ...
    for (const entry of valid) {
      try {
        await writeActorMemory({
          ...
          memoryType: typeof entry.memoryType === "string" ? entry.memoryType : "semantic",
        });
        ok++;
      } catch { fail++; }
    }
    ...
    queryClient.invalidateQueries({ queryKey: ["actorMemories", actorId] });
    if (fail === 0) toast.success(`Imported ${ok} memor${ok === 1 ? "y" : "ies"}`);
    ...
    <div className="space-y-5">
      ...
      <div className="bg-violet-500/10 border border-violet-500/20 rounded-2xl px-5 py-4">
        <div className="flex items-start gap-3">
          ...
          <p className="text-violet-200 text-sm leading-relaxed">
            ...
            <code className="font-mono text-xs bg-violet-500/20 px-1 rounded">__actor_context__</code>{" "}
            when this actor triggers a workflow with <em>inject_memory_context</em> enabled.{" "}
            <strong>Episodic</strong> (7d), <strong>Working</strong> (1h), and{" "}
            <strong>Scratchpad</strong> (24h) are time-limited.
          </p>
        </div>
      </div>
      ...
      {memories.length > 3 && (
        <div className="relative">
          ...
          />
          {memSearch && (
            <button onClick={() => setMemSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground/40 hover:text-white transition-premium">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          {memories.length > 0 && (
            <button onClick={handleExport} title="Export memories as JSON" className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-white border border-white/10 hover:border-[rgba(255,255,255,0.16)] px-3 py-1.5 rounded-lg transition-premium">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-14 gap-3">
          ...
          <button onClick={() => setShowForm(true)} className="text-xs text-violet-400 hover:text-violet-300 transition-premium">
            Add the first memory →
          </button>
        </div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-10 gap-2">
          <p className="text-muted-foreground/40 text-sm">No memories match your search.</p>
          <button onClick={() => { setMemSearch(""); setFilterType("all"); }} className="text-xs text-violet-400 hover:text-violet-300 transition-premium">
            Clear filters →
          </button>
        </div>
      ) : (
          ...
          {(Object.keys(grouped) as MemoryType[]).map((type) => (
            <div key={type}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] text-muted-foreground/40">{MEMORY_TYPE_META[type as MemoryType]?.ttl}</span>
              </div>
              <div className="space-y-2">

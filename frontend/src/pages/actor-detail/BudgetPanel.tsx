...
import { getActorExecutionsSummary, type ActorDetails, type ActorExecutionsSummary } from "@/lib/graphqlClient";
import { StatCard } from "./shared";

export function BudgetPanel({ actorId, actor }: { actorId: string; actor: ActorDetails }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-surface-3/60 border border-white/5 rounded-2xl px-6 py-5">
        <h2 className="text-white font-medium text-sm mb-4">Execution Summary</h2>
        {isLoading ? (
            ...
            {summary.totalExecutions > 0 && (
              <div className="w-full h-2 bg-background rounded-full overflow-hidden flex">
                <div
                  ...
                  style={{ width: `${(summary.successfulExecutions / summary.totalExecutions) * 100}%` }}
                />
                <div
                  className="h-full bg-red-500 transition-premium"
                  style={{ width: `${(summary.failedExecutions / summary.totalExecutions) * 100}%` }}
                />
                <div
                  className="h-full bg-sky-500 transition-premium"
                  style={{ width: `${(summary.activeExecutions / summary.totalExecutions) * 100}%` }}
                />
              </div>
            )}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatCard label="Total" value={summary.totalExecutions} />
              <StatCard label="Successful" value={summary.successfulExecutions} accent="text-emerald-400" />
              <StatCard label="Failed" value={summary.failedExecutions} accent="text-red-400" />
              <StatCard label="Active" value={summary.activeExecutions} accent="text-sky-400" />
            </div>
            {successRate !== null && (
      ...
      <div className="bg-surface-3/60 border border-white/5 rounded-2xl px-6 py-5">
        ...
        <p className="text-2xl font-bold text-white tabular-nums">
          {actor.totalBudgetUsd != null ? `$${actor.totalBudgetUsd.toFixed(2)}` : "Unlimited"}
        </p>
        {actor.totalBudgetUsd != null && (
          <p className="text-muted-foreground text-xs mt-1">
            Spent:{" "}
            <span className="text-white font-semibold">${actor.spentBudgetUsd.toFixed(2)}</span>
          </p>
        )}
      </div>
    </div>

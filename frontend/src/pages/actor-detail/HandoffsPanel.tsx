...
function parseHandoffPartner(summary: string): { name: string; id: string | null } {
  ...
  const name =
    summary
      ...
      .replace(/^(Handed off to|Received handoff from|Sent to|Received from)\s*/i, "")
      .trim() || summary;
  return { name, id };
}
...
export function HandoffsPanel({ entries }: { entries: ActorActionLogEntry[] }) {
  ...
  const handoffs = entries.filter((e) => e.actionType.toLowerCase().includes("handoff"));
  const outbound = handoffs.filter((e) => !e.actionType.toLowerCase().includes("received"));
  const inbound  = handoffs.filter((e) =>  e.actionType.toLowerCase().includes("received"));

  const partnerFreq = useMemo(() => {
    const freq = new Map<string, { name: string; id: string | null; outCount: number; inCount: number }>();
    ...
    return [...freq.values()].sort((a, b) => (b.outCount + b.inCount) - (a.outCount + a.inCount));
  }, [outbound, inbound]);

  if (handoffs.length === 0) {
    return (
      <div className="space-y-4">
        <LocalEmptyState icon={<Shuffle size={40} />} message="No handoffs recorded yet" />
        <div className="bg-surface-3/60 border border-white/5 rounded-2xl px-5 py-4">
          <p className="text-muted-foreground/40 text-xs leading-relaxed">
            Handoffs let this actor delegate work to another actor mid-execution. Use the{" "}
            <code className="text-violet-300 bg-violet-500/10 px-1 rounded font-mono text-[11px]">
              handoff_to_actor
            </code>{" "}
            MCP tool or call it from a workflow node to start building a handoff chain.
          </p>
        </div>
      </div>
    );
            ...
            {partnerFreq.map((p) => (
              <div key={p.id ?? p.name} className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3 shrink-0 text-[11px]">
                  {p.outCount > 0 && (
                    <span className="text-violet-400" title="Outbound handoffs">→ {p.outCount}</span>
                  )}
                  {p.inCount > 0 && (
                    <span className="text-emerald-400" title="Inbound handoffs">← {p.inCount}</span>
                  )}
                </div>
              </div>
            ))}
              ...
              return (
                <div key={e.id} className="flex items-start justify-between gap-4">
                  <p className="text-muted-foreground text-sm">
                    ...
                    {id ? (
                      <button onClick={() => navigate(`/actors/${id}`)} className="text-violet-300 hover:text-violet-200 transition-premium">
                        {name}
                      </button>
                    ) : name}
                  </p>
                  <time className="text-muted-foreground/40 text-[11px] shrink-0">{relativeTime(e.timestamp)}</time>
                </div>
              );
            })}
          </div>
              ...
              return (
                <div key={e.id} className="flex items-start justify-between gap-4">
                  <p className="text-muted-foreground text-sm">
                    ...
                    {id ? (
                      <button onClick={() => navigate(`/actors/${id}`)} className="text-emerald-300 hover:text-emerald-200 transition-premium">
                        {name}
                      </button>
                    ) : name}
                  </p>

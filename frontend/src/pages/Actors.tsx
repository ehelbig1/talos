...
import { CreateActorPanel, ACTOR_TEMPLATES, type ActorTemplate } from "./actors/CreateActorPanel";
...
export default function Actors() {
  ...
  const pendingCreateInput = React.useRef<Parameters<typeof createActor>[0] & { template?: ActorTemplate } | null>(null);

  const createMut = useMutation({
    mutationFn: (input: Parameters<typeof createActor>[0] & { template?: ActorTemplate }) => {
    ...
    onSuccess: async (actor) => {
      ...
      if (tmpl && tmpl.id !== "custom" && tmpl.persona?.role) {
        try {
          ...
          toast.success(`Actor "${actor.name}" created with ${tmpl.name} persona`);
        } catch {
      ...
      <header className="px-10 pt-16 pb-10 shrink-0 relative z-10">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-10 mb-12">
          <div className="space-y-4">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black text-primary/60 uppercase tracking-[0.3em]">Governance Protocol Active</span>
            ...
            <p className="text-sm text-muted-foreground/40 font-bold uppercase tracking-widest max-w-xl leading-relaxed">
              Governing <span className="text-white/60">{actors.length}</span> autonomous execution identities. Manage capability ceilings and persistent operational memory.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            {compareMode && (
              <button
                onClick={() => {
                  if (compareSet.size >= 2) {
                    navigate(`/actors/compare?actors=${[...compareSet].join(",")}`);
                  }
              ...
              >
                ...
                {compareSet.size >= 2 ? `Initialize Analytics (${compareSet.size})` : "Select Identities"}
              </button>
            )}
        ...
        {actors.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6 p-6 bg-surface-3/40 backdrop-blur-3xl border border-white/5 rounded-[2.5rem] shadow-2xl glass gpu mb-10">
            {[
              { label: "Total Population", value: actors.length, color: "text-white" },
              { label: "Active Nodes", value: actors.filter((a) => a.status === "active").length, color: "text-success", dot: true },
              { label: "Suspended", value: actors.filter((a) => a.status === "suspended").length, color: "text-warning" },
              { label: "Terminated", value: actors.filter((a) => a.status === "terminated").length, color: "text-destructive" },
              { label: "Aggregate Throughput", value: totalExecutions >= 1000 ? `${(totalExecutions/1000).toFixed(1)}k` : totalExecutions, color: "text-primary" },
            ].map((stat) => (
              <div key={stat.label} className="px-6 py-2 flex flex-col border-r border-white/5 last:border-0">
                <span className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] mb-2">{stat.label}</span>
                <div className="flex items-center gap-3">
                    {stat.dot && <div className="w-2 h-2 rounded-full bg-success animate-status-pulse shadow-[0_0_8px_hsla(var(--success),0.5)]" />}
                    <span className={cn("text-3xl font-black tabular-nums leading-none font-outfit tracking-tighter", stat.color)}>{stat.value}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col xl:flex-row items-center gap-8 p-3 bg-surface-3/40 backdrop-blur-3xl border border-white/5 rounded-[2rem] shadow-2xl glass gpu">
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 w-full xl:w-auto">
              <div className="flex items-center gap-3 pr-2">
                <span className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.3em] mr-4 whitespace-nowrap">Sequence Registry</span>
      ...
      <div className="flex-1 overflow-auto custom-scrollbar px-10 pt-2 pb-32 relative z-0 gpu optimize-blur">
        <div className="max-w-[1600px] mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                {[0, 1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="h-[320px] rounded-[2.5rem] bg-surface-3/20 border border-white/5 animate-pulse" />
                ))}
            </div>
            ) : error ? (
            <div className="flex flex-col items-center justify-center py-40 bg-surface-3/20 border border-white/5 rounded-[4rem] backdrop-blur-3xl glass-dark animate-in fade-in zoom-in-95 duration-700 max-w-4xl mx-auto">
                ...
                {sanitizeErrorMessage(error instanceof Error ? error.message : "The identity synchronization service is currently unavailable.")}
                </p>
            </div>
            ) : actors.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-40 bg-surface-3/20 border border-white/5 border-dashed border-2 rounded-[4rem] backdrop-blur-3xl glass-dark animate-in fade-in zoom-in-95 duration-700 max-w-4xl mx-auto">
                ...
                <p className="text-muted-foreground/40 text-sm max-w-xl font-bold uppercase tracking-widest leading-relaxed text-center px-12 mb-12">
                    Actors are bounded execution identities that govern automation logic,
                    enforce capability ceilings, and persist operational memory across the platform.
                </p>
                <button
                onClick={() => setShowCreate(true)}
                ...
                <details className="group border-t border-white/5 pt-20 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest opacity-40">
                            {terminatedActors.length} Decommissioned Identit{terminatedActors.length !== 1 ? "ies" : "y"} Registered
                        </span>
                    </div>
                    </summary>

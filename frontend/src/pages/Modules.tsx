...
function langColor(lang: string | null | undefined): string {
  if (lang === "rust") return "text-orange-400 bg-orange-500/10 border-orange-500/20";
  if (lang === "javascript" || lang === "js") return "text-yellow-400 bg-yellow-500/10 border-yellow-500/20";
  if (lang === "typescript" || lang === "ts") return "text-blue-400 bg-blue-500/10 border-blue-500/20";
...
function ModuleCard({ mod, onUseInEditor }: { mod: Module; onUseInEditor: () => void }) {
  const [expanded, setExpanded] = useState(false);
  const configFields = useMemo(() => parseConfigFields(mod.config), [mod.config]);
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap mb-2">
            <h3 className="text-lg font-black text-white tracking-tight font-outfit uppercase leading-none">{mod.name}</h3>
            <span className={cn("inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-[0.2em] border shrink-0 transition-premium shadow-sm", langColor(lang))}>
              {lang}
            </span>
            <span className={cn("inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-[0.2em] border shrink-0 transition-premium shadow-sm", worldColor(mod.capabilityWorld))}>
          ...
          {mod.capabilityDescription && (
            <p className="text-xs text-muted-foreground/60 font-bold leading-relaxed line-clamp-1 mb-3">{mod.capabilityDescription}</p>
          )}
            ...
            {configFields.length > 0 && (
              <>
                ...
                <span className="flex items-center gap-2">
                  ...
                  {configFields.length} Protocol Field{configFields.length === 1 ? "" : "s"}
                </span>
              </>
            )}
          </div>
        ...
        <div className="flex items-center gap-4 shrink-0">
          <button
            onClick={(e) => { e.stopPropagation(); onUseInEditor(); }}
        ...
        <div className="border-t border-white/5 px-8 py-8 space-y-8 bg-surface-4/40 animate-in slide-in-from-top-4 duration-500 glass-dark">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
            <div className="space-y-3">
              <p className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.3em]">Module Identifier</p>
            ...
            <div className="space-y-3">
              <p className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.3em]">Integrity Hash</p>
...
export default function Modules() {
  ...
  const filtered = useMemo(() => {
    let result = modules;
    if (worldFilter) result = result.filter((m) => m.capabilityWorld === worldFilter);
    if (search.trim()) {
    ...
    <div className="px-10 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-3xl font-black text-white tracking-tighter font-outfit uppercase leading-none mb-1.5">Active Library</h1>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <RefreshCw className={cn("w-5 h-5 group-hover:rotate-180 transition-transform duration-700", isFetching && "animate-spin")} />
          </button>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? worldColor(w).replace("border-", "border-").replace("bg-", "bg-") + " text-white"
                    : "text-muted-foreground/40 border-white/5 hover:text-white hover:bg-white/5",
                )}
              >
      ...
      {isLoading ? (
        <div className="space-y-6">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-32 bg-surface-3/20 border border-white/5 rounded-[2rem] animate-pulse" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="space-y-6">
          {filtered.map((mod) => (
            <ModuleCard
              key={mod.id}
              mod={mod}
              onUseInEditor={() => { clearWorkflow(); navigate(`/editor?moduleId=${mod.id}`); }}
            />
          ))}
        </div>
      )}

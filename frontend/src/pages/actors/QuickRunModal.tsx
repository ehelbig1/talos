...
import { graphqlRequest, getActorWorkflows, type ActorSummary, type ActorWorkflowItem } from "@/lib/graphqlClient";
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="bg-success/5 border border-success/20 rounded-[1.5rem] p-6 glass-light shadow-xl relative group overflow-hidden">
                ...
                <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-success text-[10px] font-black uppercase tracking-[0.2em]">Protocol Sequence Synchronized</p>
                </div>
                <div className="space-y-1">
                    <p className="text-muted-foreground/40 text-[9px] font-bold uppercase tracking-widest">Execution Trace Identifier</p>
                    <p className="text-white font-mono text-xs break-all selection:bg-success/30">{result.execId}</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="space-y-8">
              {isLoading ? (
                ...
                <div className="py-10 text-center bg-white/2 rounded-2xl border border-white/5 border-dashed">
                    <p className="text-muted-foreground/40 text-[10px] font-black uppercase tracking-[0.2em]">No Published Protocols Detected</p>
                </div>
              ) : (
                <div className="space-y-3">
                    <label className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.3em] ml-1">Select Execution Logic</label>
                    <div className="relative group/select">
                        ...
                        >
                            <option value="" className="bg-surface-3">—— IDENTIFY SEQUENCE ——</option>
                            {published.map((w) => (
              ...
              <button
                onClick={() => selectedWorkflowId && triggerMut.mutate(selectedWorkflowId)}

    ...
    <span
      className={cn(
        ...
        size === "sm"
          ...
          : "text-[9px] px-2.5 py-0.5 shadow-sm",
      )}
    >
      {cfg.label}
    </span>
  ...
  return (
    <div
      className={cn(
        ...
        compareMode &&
          selectedForCompare &&
          "ring-2 ring-primary shadow-[0_0_30px_hsla(var(--primary),0.2)]",
    ...
    >
      ...
      <div className="flex items-start justify-between gap-5 relative z-10">
        <div className="min-w-0 flex-1 space-y-2">
          ...
          {actor.description ? (
            <p className="text-muted-foreground/60 text-xs font-bold line-clamp-1 leading-relaxed uppercase tracking-wide">
              {actor.description}
            </p>
          ) : (
            <p className="text-muted-foreground/20 text-[10px] font-black uppercase tracking-[0.3em] italic leading-none">
              ID Unclassified
            </p>
          )}
        </div>
        ...
        >
          <span
            className={cn(
              ...
              colors.dot,
              actor.status === "active" &&
                "animate-status-pulse shadow-[0_0_8px_currentColor]",
            )}
          />
          {actor.status}
        </span>
        ...
        {[
          {
            ...
            icon: GitBranch,
            accent: "group-hover/m:text-primary",
          },
          {
            label: "Cycles",
            value:
              actor.executionCount >= 1000
                ? `${(actor.executionCount / 1000).toFixed(1)}k`
                : actor.executionCount,
            icon: Zap,
            accent: "group-hover/m:text-success",
          },
          {
            label: "Activity",
            value: relativeTime(actor.updatedAt),
            icon: Clock,
            accent: "group-hover/m:text-warning",
          },
        ].map((s) => (
          <div
            key={s.label}
            className="bg-surface-4/40 border border-white/5 rounded-2xl p-4 text-center transition-premium hover:bg-surface-4/60 hover:border-white/20 glass-light group/m shadow-sm"
          >
            ...
            <div className="text-sm font-black text-white font-outfit tracking-tighter truncate">
              {s.value}
            </div>
          </div>
        ))}
      ...
      <div className="mt-auto pt-6 border-t border-white/5 flex items-center justify-between gap-4 relative z-10">
        <span className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-[0.3em]">
          Provisioned{" "}
          <span className="text-white/20">{relativeTime(actor.createdAt)}</span>
        </span>

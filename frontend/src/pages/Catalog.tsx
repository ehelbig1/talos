    ...
    <div className="group flex flex-col bg-surface-3/40 backdrop-blur-md border border-white/5 rounded-[2rem] p-8 hover:border-primary/30 transition-premium shadow-2xl glass relative overflow-hidden h-full">
      ...
      <div className="flex items-start justify-between gap-4 mb-6 relative z-10">
        <div className="w-14 h-14 rounded-2xl bg-surface-4/60 border border-white/10 flex items-center justify-center shrink-0 shadow-2xl transition-premium group-hover:scale-110 group-hover:border-primary/30 relative">
          ...
          <div className={cn("relative z-10 transition-colors", meta.textColor)}>
            {meta.icon}
          </div>
        </div>
      ...
      <p className="text-xs text-muted-foreground/50 font-bold leading-relaxed line-clamp-2 flex-1 mb-8 relative z-10">
        {template.description ?? "Standard execution protocol for distributed operations."}
      </p>
...
function InstallDialog({ template, onClose }: InstallDialogProps) {
  ...
  const [config, setConfig] = useState(() => {
    try {
      const schema = JSON.parse(template.configSchema) as { properties?: Record<string, unknown> };
  ...
  const handleConfigChange = (v: string) => {
    setConfig(v);
    try { JSON.parse(v); setConfigError(null); } catch { setConfigError("INVALID JSON STRUCTURE"); }
  };
      ...
      <Dialog open={true} onClose={onClose} title="Module Initialized">
        <div className="text-center p-6">
            ...
            <div className="relative z-10 mb-10">
                ...
                <span className="text-white">{name}</span> HAS BEEN SYNTHESIZED AND ADDED TO YOUR ACTIVE LIBRARY.
                </p>
            </div>
    ...
    <Dialog open={true} onClose={onClose} title="Initialize Protocol">
      <div className="space-y-10 relative z-10 p-2">
        <div className="relative z-10 -mt-6 mb-4">
          <p className="text-[10px] font-black text-primary/60 uppercase tracking-[0.4em] leading-none">{template.name}</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          />
          {configError && <p className="text-[9px] font-black text-destructive mt-3 uppercase tracking-[0.2em]">{configError}</p>}
        </div>
        ...
        {template.allowedHosts.length > 0 && (
          <div className="bg-warning/5 border border-warning/20 rounded-[1.5rem] px-6 py-5 shadow-2xl relative overflow-hidden group">
            ...
            <p className="text-[10px] font-black text-warning uppercase tracking-[0.3em] mb-2 flex items-center gap-3">
              <Globe className="w-4 h-4 opacity-60" /> Security Policy: Network Egress Authorized
            </p>
            <p className="text-[11px] text-warning/60 font-bold leading-relaxed uppercase tracking-widest">
              ...
              <span className="font-mono text-white/80 underline decoration-warning/30 underline-offset-4">{template.allowedHosts.join(", ")}</span>
            </p>
          </div>
        )}
        ...
        <div className="flex items-center justify-end gap-6 pt-6 border-t border-white/5">
          ...
          <Button
            variant="premium"
            onClick={() => mutation.mutate({ input: { templateId: template.id, name: name.trim(), config } })}
...
export default function Catalog() {
  ...
  const [installTemplate, setInstallTemplate] = useState<NodeTemplate | null>(null);
    ...
    <div className="px-10 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-3xl font-black text-white tracking-tighter font-outfit uppercase leading-none mb-1.5">Registry Catalog</h1>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? cn(meta.textColor, meta.bgColor, meta.borderColor) + " text-white"
                    : "text-muted-foreground/40 border-white/5 hover:text-white hover:bg-white/5",
                )}
              >
                ...
                {cat}
                <span className={cn("ml-1 tabular-nums", isActive ? "opacity-30" : "opacity-20")}>
                  {count}
                </span>
              </button>
            );
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-64 bg-surface-3/20 border border-white/5 rounded-[2rem] animate-pulse" />
          ))}
        </div>
      ) : search.trim() ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              ...
              return (
                <section key={cat} className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                  <div className="flex items-center gap-6 mb-10 relative group">
                    <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center shadow-2xl transition-premium group-hover:scale-110", meta.bgColor, meta.textColor)}>
                      {meta.icon}
                    </div>
                    <div>
                      ...
                      <p className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.3em] leading-none">
                        {items.length} Sequence Protocol{items.length !== 1 ? "s" : ""} Synchronized
                      </p>
                    </div>
